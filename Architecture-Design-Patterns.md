...
**Entity** - объект, имеющий уникальные черты. Изменяемый. Имеет уникальный идентификатор (id). Два Entities считаются разными даже в случае совпадения всех свойств.

**Value-объект** (Value Object) - неизменяемый (immutable) тип объекта, полностью определяемый своими свойствами. В отличие от Entity не имеет уникального идентификатора (id). Два Value-объектас одинаковыми свойствами считаются идентичными.

Пример Entity и Value-объекта. Несмотря на то, что в кинотеатре все места выглядят одинаково, каждое место имеет свой номер (уникальный идентификатор). Посетитель приходит и садится на место, указанное в его билете. Такие места - Entity. В общественном транспорте все места одинаковы: можно сесть куда угодно, нумерация не имеет значения. Такие места - Value-объекты.
