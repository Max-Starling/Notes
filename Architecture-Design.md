...
Специализация применяется, если определённые характеристики (атрибуты, методы и ассоциации) применяются только к некоторым объектам класса. В таком случае может быть создан специальный подкласс (special/specific subclass).

Специализация может достигаться не только за счёт того, что в подклассе есть поля, которых нет в суперклассе, но и за счёт того, что в суперклассе некоторые поля могут быть фиксированы.

Специализация может достигаться не только за счёт наследования, но и за счёт композиции.

Пример специализации за счёт композиции на примере React.
