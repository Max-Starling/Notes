...
* Как много уровней (tiers) будет в нашей клиент-серверной архитектуре? Как они будут взаимодействовать?
...
* Нужно за чем-то проследить в приложении и отреагировать соответствующим образом (например, показать уведомление при появлении пользователя в сети) — *поведенческий паттерн "Наблюдатель"*.
...
HMVC разбивает уровень клиента (client tier) на иерархию из MVC-слоёв. Это называется **проектированием клиентского уровня** (client-tier architecture) и должно увеличивать масштабируемость приложения: каждый MVC-слой независим от других и может работать при отсутствии любого другого.
...
В случае, когда два View зависят друг от друга (input — один View, кнопка — другой), нужно ViewModel одного View подписаться на событие, которое излучает другой ViewModel. 
...
**View** — шаблон (template), на основании которого генерируется HTML при помощи шаблонизатора (template engine).
...
Любое *взаимодействие с системой* происходит через *Boundary*. Действующее лицо (actor) может быть не только человеком, но и другим устройством или сторонним API.
...
EBI стремится к тому, чтобы бизнес-логика приложения, представленная Entities и Interactors, была независима от механизма доставки. Эта логика не должна знать, что её использует. Это позволяет писать на любом языке для любого случая использования (use case). Требуется лишь умение определять абстракции наподобие интерфейсов. Причём статическая типизация здесь не нужна: нужен только способ создания чётких и проверяемых определений функциональности. Если необходимой структуры нет, то достаточно написания документации. Boundary должен быть уточнён (be specified), но не важно, каким образом.
...
### Уровни
* **Уровень представления** (Presentation Tier) содержит пользовательский интерфейс (UI). Соответствует фронтенду (HTML, JS, CSS, фреймворки и прочее).
...
В корпоративных (enterprise) системах, где работает огромное количество разработчиков, модель может сильно разрастись. Сложно держать всю структуру огромного проекта в голове. Сложно распределять обязанности, когда все работают в одном месте. В этом случае систему обычно разбивают на подсистемы. 
...
Если в приложении есть общие подобласти, нужно уделять им как можно меньше внимания по сравнению с предметной областью. Это не самая важная часть приложения, а если и важная, то не критичная (essential but not crucial). 
...
Чтобы показать, что приложение имеет много точек входа/выхода, приложение представляется в виде многоугольника (одна сторона — одна точка). Несмотря на то, что сторон может быть сколько угодно, архитектуру назвали шестиугольной (hexagonal).
...
**Вторичный, управляемый адаптер** (Secondary, Driven Adapter) — адаптер правой стороны схемы, отвечающей за подключение бэкенд-технологий. Реагирует на действие первичного адаптера.
...
Для реализации одиночки пишется класс с приватным конструктором; статическим полем, хранящим экземпляр (instance) класса, и статическим методом, создающим и сохраняющим экземпляр при первом обращении и возвращающим при последующих.
```ts
class Singleton {
...
**Фабричный метод** (Factory Method) — порождающий паттерн, определяющий интерфейс для создания объекта некоторого класса, при этом решение о том, какой класс будет у объекта, принимается в дочерних классах.
...
**Связанность, зацепление** (coupling) отражает зависимость блоков кода друг от друга. Два блока кода **сильно связаны** (highly coupled), если изменения в одном блоке порождают изменения в другом, **слабо связаны** (loosely coupled), если один блок не зависит или почти не зависит от изменений другого. 

**Сплочённость, связность** (cohesion) отражает, насколько тесно связаны по смыслу функции одного модуля. **Низкая сплочённость** (low cohesion) характерна модулям, имеющим разные несвязанные обязанности, **высокая сплочённость** (high cohesion) — модулям, функции которых выполняют во многом похожие задачи.
...
* **Reuse-Release Equivalency (RRE)**. Пакеты должны выпускаться и версионироваться отдельно друг от друга, чтобы всегда можно было выбрать старую версию пакета, если новая некорректно работает.
* **Common-Reuse Principle (CRP)**. Классы в пакете переиспользуются (reuse) вместе. Переиспользуешь один — переиспользуешь все.
* **Common-Closure Principle (CCP)**. Классы, изменяющиеся вместе, должны быть упакованы (packaged) вместе. Чтобы в случае изменений изменялся только один пакет, не затрагивая другие.
...
Пусть есть два класса: `Employee`, отвечающий за рабочих, и `Salary`, отвечающий за заработную плату. 
...
*Существующий* класс в этом случае называют **базовым** (base), **родительским** (parent), **суперклассом** (superclass).
...
**Агрегация** (Aggregation) является формой ассоциации, когда один объект "имеет" (отношение "has-a") ссылку на другой объект. Содержащий объект иногда называют **контейнером**.
...
экземпляр компонента Title создаётся вместе с ним. */
...
**Наследование** (Inheritance) — механизм, передающий характеристики суперкласса его подклассам. Подклассы **наследуют** характеристики своего суперкласса.

Пример *обобщения*: собака и лошадь являются домашними животными, самолёт и машина являются транспортными средствами.

*Обобщение* использует *отношение "is-a" (является)*.

*Horse is a Domestic Animal, Dog is a Domestic Animal* (класс `DomesticAnimal` — *обобщённый суперкласс*, `Horse`, `Dog` — его *подклассы*).
```js
...
```
Хорошо подумав, мы решаем, что одного класса для нашего приложения мало: необходимы более конкретные виды птиц (чайка, голубь). Помечаем класс `Bird` как абстрактный, чтобы нельзя было создавать экземпляры "неконкретных" птиц (`new Bird()`), а также помечаем некоторые его методы (в приложении чайка и голубь могут спать и есть по-разному, пусть в них это и определяется).
```ts
abstract class Bird {
  age: number;
  size: number;
...
```
Затем мы решаем, что есть плавающие и летающие птицы.
```ts
abstract class Bird { /* ... */ }
...
Вероятно, при использовании композиции часть кода будет дублироваться, но он будет менее связанным, а значит более поддерживаемым.
...
* **Тестирование**. Композицию можно просто подменить тестовым объектом (заглушкой, реализующей интерфейс), наследование требует использования родительского класса, заменить который нельзя и придётся напрямую его импортировать.
...
Если класс отвечает за решение нескольких задач, его подсистемы, реализующие решение этих задач, оказываются сильно связаны друг с другом. Изменения в одной такой подсистеме ведут к изменениям в другой.
...
Его идея заключается в том, что следует писать код, который будет достаточно гибким для изменений в будущем. Новый функционал должен вносить минимум изменений в уже существующий. Но обычно новый функционал не всегда просто предсказать. 

Пытаясь продумать многие случаи использования наперёд, разработчик часто создаёт такие вещи, которые в данный момент ему не нужны, а в будущем могут даже и не понадобиться. При этом они увеличивают сложность кода, требуют времени на написание и поддержку. Нелегко найти баланс между написанием гибкого кода и написанием ненужного кода, поэтому у принципа открытости-закрытости есть противоположный принцип - принцип YAGNI.

Пусть необходимо написать класс, описывающий личного ассистента с возможностью приветствия.  
...
Пусть помимо слов в приветствии, появляется дополнительное действие (например, помахать рукой), тогда можно создать новый метод, не затрагивая старый код.
```ts
...
Удалось сделать достаточно гибкий к изменениям класс. Если в дальнейшем понадобится дополнительно добавить локализацию, заменить фразу, считать фразы из файла или получать их по запросу, потребуется минимум изменений в уже написанном коде.
...
Класс `Bird` имеет метод `fly` (летать), `Chicken` наследует этот метод. Тем не менее, пример ниже выдаст ошибку.
```ts
...
Одним из способов это сделать является перенос метода `fly()` из `Bird` в новый подкласс `FlyingBird`. В этом случае используется принцип разделения интерфейса. 
```ts
class Bird {
  makeSound(): void {};
}
...
**Принцип KISS** (keep it short and simple) переводится как "Держи это коротким и простым" и утверждает, что в большинстве случаев система работает лучше, если остаётся простой, поэтому в области проектирования простота должна быть одной из ключевых целей и следует избегать ненужной сложности.
