...
Необходимо, чтобы дочерний класс, подкласс (subclass) мог служить заменой своего родительского класса, суперкласса (superclass).

Пусть есть курица и птица, курица является птицей. Есть класс `Bird` и класс `Chicken`, наследующий от него.
```ts
...
```
Класс `Bird` имеет методы `fly` (летать) и `makeSound` (издать звук), класс `Chicken` переопределяет метод `makeSound`.

Р.Мартин определил принцип Бардары Лисков следующим образом:  
> Функции, которые используют базовый тип, должны иметь возможность использовать подтипы базового типа, не зная об этом.

Возвращаясь к примеру:  `animateFlight(bird: Bird)` — упомянутая функция, `Bird` — базовый тип, `Chicken` — подтип. Опираясь на высказывание, функция `animateFlight` не должна знать о том, что использует класс `Chicken` вместо `Bird`, что означает, что в ней не должно быть проверки принадлежности к классу (например, `if (bird instanceof Chicken)`).

Пример ниже выдаст ошибку, поскольку *курица не умеет летать*.
```ts
animateFlight(new Bird());
animateFlight(new Chicken()); // ошибка
```
Такую ошибку так или иначе придётся обрабатывать где-то выбрасыванием исключения (например, при попытке подгрузить несуществующую анимацию полёта). То есть мы не можем использовать функцию `animateFlight()` с `Chicken`, но можем использовать с классом `Bird`, от которого она наследуется. Это свидетельствует о плохо подобранной абстракции, нарушающей принцип Барбары Лисков.

Чтобы принцип соблюдался, следует убрать метод `fly` из класса `Bird` (поскольку не всем птицам характерно это поведение), например, в класс `FlyingBird` (от принципа разделения интерфейса это отличается тем, что нам не важно, что станет с методом `fly`). 
```ts
class Bird {
  makeSound(): void {};
}
...
```
Теперь полёт не является обязательной характеристикой птицы, поэтому нам следует поменять тип параметра у функции `animateFlight` на `FlyingBird`. 
```ts
const animateFlight = (bird: FlyingBird) => { /* ... */ };
```
Теперь функция не может работать с `Chiken`, но она не может работать и с `Bird`, что не нарушаешь принципа Барбары Лисков. Зато если мы создадим класс `Dove`, описывающий голубя, как наследник класса `FlyingBird`, то код ниже отработает без ошибок.
```ts
