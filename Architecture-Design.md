...
const animateFlight = (bird: Bird) => {
  /* подгружается анимация полёта птицы и  */
...
```
Класс `Bird` имеет методы `fly` (летать) и `makeSound` (издать звук).

Р.Мартин определил принцип Бардары Лисков следующим образом:  
"Функции, которые используют базовый тип, должны иметь возможность использовать подтипы базового типа, не зная об этом."

Функция, о которой идёт речь, — `animateFlight(bird: Bird)`, базовый тип — `Bird`, подтип — `Chicken`, 
...
```
Такую ошибку так или иначе придётся обрабатывать где-то выбрасыванием исключения. То есть мы не можем использовать функцию `animateFlight()` с `Chicken`, но можем использовать с классом `Bird`, от которого она наследуется. Это свидетельствует о плохо подобранной абстракции, нарушающей принцип Барбары Лисков.
