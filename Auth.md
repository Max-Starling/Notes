...
Например, можно *идентифицировать* человека по имени, электронной почте, номеру телефона или паспорта.
...
`UUID` (англ. `Universally Unique IDentifier`) — *стандарт идентификации*, стандартизированный Open Software Foundation как часть среды распределённых вычислений (DCE). 
...
```
В JavaScript можно кодировать и декодировать Base64 следующим образом.
```js
...
**Токены доступа** (Access Tokens) представляют собой учётные данные (credentials), которые используются для доступа к защищенным ресурсам.
...
Токены обновления не хранят в себе какую-то информацию, как JWT-токены, и обычно представляют собой обычную хеш-строку. Токены обновления необходимо хранить на сервере.
