...
```
**Виртуальное окно** (virtual window), **вьюпорт** (viewport) - область окна, в которой виден контент.  
...
Значение может быть как фиксированным `width=700`, так и специальным `width=device-width`, которое адаптируется под экран пользователя.  По умолчанию `width=980`.
...
Свойства `maximum-scale`, `minimum-scale`, `user-scalable` отвечают за возможности масштабирования страницы пользователем.  
