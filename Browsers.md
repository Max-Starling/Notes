...
**Обработка значения** (Value Processing):

* **Объявленное значение** (Declared value). Значение, объявленное автором (atuhor decllarations).
* **Каскадное значение** (Cascaded value). Значение, полученное в результате применения каскадирования. Применяется более приоритетный стиль, если есть конфликтующие.
* **Указанное значение** (Specified value). Значение, заданное по умолчанию для свойства (initial value). Каждое свойство имеет указанное значение, которое используется, если каскадное значение отсутствует.
