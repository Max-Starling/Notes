...
- [Как всё устроено](#как-всё-устроено)
  ...
  - [Расчёт макета (Layout, Reflow)](#расчёт-макета-layout-reflow)
...
**Файл** (англ. `file`) — определённое количество информации (в виде программы или данных), имеющее имя и хранящееся во внешней памяти компьютера.

**Текстовый** файл (англ. `text file`) содержит *текстовые данные* (символы, составляющие слова, формулы и прочее).
...
Если отсутствует протокол и браузеру не удаётся найти корректный hostname в строке, то она воспринимается как поисковый запрос.  
...
На самом деле полный домен выглядит вот так `wikipedia.org.`, символ `.` в конце — **корневой домен**, `org` — домен верхнего (первого) уровня, `wikipedia` — домен второго уровня.  
...
Функция `gethostbyname` сперва проверяет локальный файл `hosts` (текстовый файл, содержащий базу данных доменных имён и используемый для их трансляции в IP-адреса), содержимое которого задаётся администратором компьютера.
```css
/* файл hosts */
127.0.0.1       localhost
```
...
Если в кэше домена нет, то делается запрос к корневому DNS-серверу `.` (система из множества серверов по всему миру, управляющая интернетом).
...
Браузер выполняет DNS-запрос по протоколу UDP, являющемуся транспортным протоколом без установки соединения (connectionless).
...
* **Каскадное значение** (Cascaded value) — значение, полученное в *результате применения каскада*. Если есть *конфликтующие объявления*, *применяется более приоритетное* из них и становится *каскадным*. *Значение*, заданное *браузером* (user-agent declaration), *становится каскадным*, если *объявленное значение отсутствует*. Примеры *браузерных значений*: синий цвет и подчёркивание ссылок `<a>`.
...
* **Фактическое значение** (Actual value) — финальное значение, готовое для использования в макете. Вычисляется на этапе рендеринга страницы. Используемое значение может быть значением с плавающей точкой. Браузер не может разделить пиксели, поэтому округляет значение до целого `360px`.

### Перевод относительных величин и процентов в пиксели

`%` технически не является единицей измерения (unit).
...
```
* `%` для `width`, `margin`, `padding` вычисляется относительно `width` родительского элемента, `height` — относительно `height` родительского элемента, `border` *не вычисляется* в *процентах*.
```css
.parent {
  width: 200px;
  height: 100px;
...
4) Из рассмотрения *исключаются вершины*, которые *скрыты* при помощи *CSS*: имеют *объявление* `display: none`.  
5) Каждая оставшаяся вершина DOM вместе со своими наборами правил становится вершиной дерева рендеринга.  
...
## Расчёт макета (Layout, Reflow)
При **расчёте макета** (Layout) *вычисляются расположение элементов на экране* и *занимаемое* ими *место*, то есть их *геометрия*.  
...
```
Значения высоты и ширины вьюпорта берутся как базовые, затем браузер обходит (traverse) дерево рендеринга, начиная от его корня `<html>`, вычисляя все значения элементов: часто значения дочерних элементов зависят от родительских.
...
После расчёта макета каждый элемент на странице имеет свою блочную модель (box model), а все величины измерения переводятся в пиксели.  
