...
Доменные имена разделяются точками. Разрешённые символы в доменном имени: `a-z`, `0-9`, `-`. 

![DNS](./assets/DNS.png)

Читаются слева направо от младших доменов к доменам высшего уровня (уровни считаются справа налево, поскольку в древесной структуре чем ниже уровень, тем старше): .

```subdomain.domain.com```:
`subdomain` - домен третьего уровня, `domain` - домен второго уровня, `com` - домен первого уровня, `.` - корневой домен
