...
* **Объявленное значение** (Declared value). Значение, объявленное автором (atuhor decllarations).
* **Каскадное значение** (Cascaded value). Значение, полученное в результате применения *каскада*. Если есть *конфликтующие стили*, *применяется более приоритетный* из них.
* **Указанное значение** (Specified value). Значение, заданное по умолчанию для свойства (initial value). Каждое свойство имеет указанное значение, которое используется, если каскадное значение отсутствует. Значение по умолчанию не является значением, заданным браузером (user agent declaration). Значение, заданное браузером, является каскадным, если объявленное значение отсутствует. Наследуемое значение также становится указанным (применяется, если объявленное и каскадное отсутствуют).
* **Вычисленное значение** (Computed value). Значение, переведённое из относительных единиц (relative) в абсолютные (absolute).
* **Используемое значение** (Used value). Проценты переводятся в пиксели.
* **Фактическое значение** (Actual value). Происходит на этапе этапе отрисовки страницы. Используемое значение может быть значением с плавающей точкой. Например, `359.7px`. Браузер не может поделить пиксели и отрисовать это значение, поэтому округляет его: `360px`.
