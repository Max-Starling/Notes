...
**Домен** (domain) — узел в дереве имён, вместе со всеми подчинёнными (дочерними) ему узлами.  

Доменные имена читаются слева направо от младших доменов к доменам высшего уровня (

**Поддомен** (subdomain) — подчинённый домен.  
`domain.com` - поддомен домена `com`, `subdomain.domain.com` - поддомен домена `domain.com`.

**Domain Name System** (DNS) — система доменных имён; иерархичесая децентрализованная система именования, хранящая информацию о доменах. Обычно её используют для получения IP-адреса по имени хоста.
...
Если файл `hosts` не содержит нужного домена, делается запрос к DNS-серверу, являющегося частью настроенного стека протоколов (network stack).
