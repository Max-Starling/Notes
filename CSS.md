...
**Заявление** (statement) — *структурный элемент CSS*, который *начинается* с *любых непробельных символов* и *заканчивается первой закрывающей фигурной скобкой* или *точкой с запятой*.
...
Перед тем, как отрендерить страницу, браузер

**Байты** (bytes) -> **Символы** (characters) -> **Токены** (tokens) -> Узлы (nodes) -> Объектная модель (object model)

Первым делом браузер обрабатывает HTML:
1) **Преобразование байтов в символы**. Браузер преобразует байты из HTML-файла, полученного с диска или из сети, в символы с учётом заданной в файле кодировки (например, UTF-8).
2) **Лексический анализ (токенизация)**. Браузер конвертирует строки символов в отдельные распознанные группы — лексемы. Например, `<html>`, `<div>`. Каждая из них имеет свой смысл. Затем эти лексемы преобразовываются в объекты - токены, имеющие определённые свойства и правила.
3) **Построение объектной модели**. Поскольку HTML-разметка определяет отношения между различными тегами (одни теги вкладываются в другие), браузер может представить её в виде дерева из токенов, которые становятся узлами дерева и, в зависимости от вложенности, устанавливают между собой дочерне-родительские отношения.
