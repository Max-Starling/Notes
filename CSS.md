...
*Псевдо-элементы* позволяют контролировать элементы, находящиеся *за пределами документа*; позволяют ссылаться на *недоступную* для получения *иным путём* информацию. Они должны всегда начинаться с двух двоеточий, хоть в некоторых браузерах их можно использовать с одним.

Примеры псевдо-элементов:
```
.field::placeholder { color: #8ab7e5 } // изменение цвета замещающего текста для input и textarea
p::before, p::after { content: "♥"; } // вставка сердечка перед контентом каждого блока p в документе и после него 
...
*::-webkit-scrollbar-thumb { background: #8ab7e5 } // изменение цвета ползунка скроллбара в браузерах на движке webkit
```
