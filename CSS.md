...
```
* *Миксины* создают *повторяющийся код*, поскольку просто *вставляют своё содержимое*.  
```css
.button {
  background: #24292e;
  color: #fff;
...
```
*Шаблонные селекторы* выносят код в отдельную *группу селекторов*, *не создавая дубликатов* кода.
```scss
