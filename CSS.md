...
Правила специфичности не так сложны при рассмотрении какого-то конкретного селектора. Но когда их тысячи в одном файле, трудно слёту рассчитать, какой стиль будет применён в итоге или как конкретный селектор повлияет на элементы страницы.

Есть некоторые правила, соблюдение которых поможет не только значительно улучшить читабильность кода, но иногда и тем или иным образом оптимизировать его:
...
### Почему лучше избегать использования селекторов по id

* Есть есть лишь селекторы по классу, тогда мы имеем абсолютное равенство между ними: специфичность вычисляется довольно просто.
```css
.page {} // 10
.content .title {} // 10 + 10 = 20
.content .title:hover {} // 10 + 10 + 1 = 21
```
Когда есть только классы, стили одного элемента обычно последовательно перезаписывают друг друга вслучае конфликтов. В примере ниже специфичность совпадает, поэтому применится стиль, располагающийся ниже в коде.
```css
.page .title {} // 10 + 10
.content .title {} // 10 + 10 = 20 (применится этот)
```
В случае же использования идентификаторов, количество возможных комбинаций возрастает, а код становится менее последовательным:
```css
...
* Один элемент не может иметь два идентификатора, но может иметь несколько классов.
```html
<p id="title header-title"></p> // это не будет работать!
<p id="title" id="header-title"></p> // это тем более! игнорирование или ошибка
<p class="title header-title"></p> // это работает
```
* Селектор по id выбирает лишь первый элемент на странице.
```html
<div>
    <p id="title"></p>
    <p id="title"></p> // игнорирование или ошибка (идентификатор должен быть уникален)
</div>
```
