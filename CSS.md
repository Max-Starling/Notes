...
- **Важность** (Importance).  
Производится *упорядочивание объявлений* по **важности** (importance), зависящей от *источника* (origin). Чем *ниже по списку ниже*, тем *выше приоритет*.
...
- **Порядок источника** (Source Order, Origin Order).  
