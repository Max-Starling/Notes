...
1) Из рассмотрения *исключаются вершины DOM*, которые не видны на странице: `<head>` со своим содержимым, `<script>` и прочие подобные.  
2) Для каждой оставшейся вершины DOM находятся соответствующие наборы правил в CSSOM.  
3) Из рассмотрения исключаются вершины, которые скрыты при помощи CSS: имеют объявление `dispalay: none`.  
4) Каждая оставшейся вершина DOM вместе со своими наборами правил становится вершиной дерева рендеринга.  

Вершина дерева рендеринга называется **фреймом** (frame) или **боксом** (box).  
