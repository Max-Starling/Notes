...
Говорят, что *потомки наследуют свойства предка*. *Наследуются не все свойства*.
...
Если добавить *следующее правило* в примере выше, то текст станет *чёрным* несмотря на то, что *специфичность* селектора `*` — `0`, а селектора `div` — `1`.
```css
* {
  color: #000;
}
