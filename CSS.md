...
Величина внешних отступов может быть отрицательной, величина остальных составляющих блочной модели — нет.  
```css
.class {
  margin-top: -16px;
}
...
*Селектор* (selector) используется для *идентификации элементов страницы*, к которой применяется *таблица стилей*.
