...
* *Остальные атрибуты* (в том числе и *class*), а также *псевдоклассы* (кроме `:not`, имеющего *нулевую специфичность*). Специфичность: *10*.
...
**Наследование** (Inheritance) — *автоматическое применение некоторых свойств элемента-предка* к его *потомкам*.

Говорят, что *потомки наследуют свойства предка*. В первую очередь наследуются свойства, определяющие параметры отображения текста.
```html
<div>
  <p>Notes</p>
</div>
...
*Наследование* — одна из причин использования *древовидной структуры* в *объектной модели CSS*.

*Наследуемые свойства не имеют  специфичности*, поэтому их может *перекрыть* даже *универсальный селектор* `*`, имеющий *нулевую специфичность*.

*Наследуются не все свойства*. Например, *не наследуются свойста блочной модели* (`border`, `margin`, `padding`, `width`, `height`), `background`, `transform` и многие другие.

Чтобы *вручную унаследовать родительское свойство*, можно использовать *значение* `inherit`.
```css
p {
  color: inherit;
}
...
*CSS* переводится как *каскадные таблицы стилей* и *ключевым* словом выступает *каскад* (cascade).

**Каскадирование** (Cascading) — *одновременное применение нескольких правил* к *элементу*. Среди *объявлений*, *принадлежащих* этим *правилам*, могут быть *конфликтующие*, то есть *содержащие* одни и *те же свойства*.
...
*Таблицы стилей* (style sheets) могут иметь *3* разных *источника* (origins): *стили браузера, *автора* (разработчика) и *пользователя*.
...
Для начала *находятся все объявления* (declarations), принадлежащие *рассматриваемому элементу*.

Затем производится *упорядочивание объявлений* по *важности* (importance) и *источнику* (origin). Чем *ниже по списку*, тем *выше приоритет*:
...
Если *важность* и *источник совпадают*, то сравнивается *специфичность селекторов* (specificity), которым принадлежат *объявления*.

В примере выше *применится* объявление `width: 200px`, поскольку его *селектор специфичнее* (подробнее об этом дальше).

Если *важность*, *источник* и *специфичность совпадают*, приоритет объявлений зависит от их расположения в коде. *Объявленный позже блок* кода имеет *больший приоритет*.
```css
p {
  color: green;
}
...
.content .title:hover {} /* 10 + 10 + 10 = 30 */
```
