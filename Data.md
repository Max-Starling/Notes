...
*Преобразование* (*отображение*) *символа алфавита* `A` в *последовательность символов алфавита* `B` называют **кодом** (англ. code). *Если алфавит* `B` *состоит лишь* из *нулей и единиц*, то есть `B = { 0, 1 }`, то *преобразование* называют **бинарным кодом** (англ. binary code). 
...
Пусть *имеются* *алфавиты* `A` и `B`. *Алфавит* `A` назовём **исходным алфавитом** (англ. source alphabet), а *алфавит* `B` - **целевым алфавитом** (англ. target alphabet).
...
*Кодировки с малым количеством бит* обладают *преимуществом* в своей *простоте* и *легковестности кода* (`1 символ = 1 байт`).

*С другой стороны* они сильно *ограничены*, ведь *однобайтовая кодировка может вместить лишь несколько языков*. 

### Unicode


Обычно для *каждого разговорного языка* имеется *своя кодировка*, которая также *включает набор основных специальных символов* и *цифры*. Это может быть *удобно* в *рамках* одной *локальной системы*, где *всё* на *одном языке*, но совершенно *не удобно*, когда *много различных систем взаимодействуют* друг с другом, а именно это и подразумевает *интернет*. Так, например, текстовый файл, напечатанный на одном компьютере, невозможно было прочитать на другом при отсутствии требуемой кодировки.
