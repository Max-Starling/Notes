...
В *программировании* символы `<`, `<= `==`, `>=`, `>` *являются операторами* (не *отношениями*, как в *теории множеств*). *Эти операторы сравнивают два значения* и *возвращают значение* *логического типа*.
...
**Размер строки** зависит от *длины* этой *строки* (*количества символов* в ней), от *размера одного символа* (*размер задаётся кодировкой*) и от *способа представления строки* в *памяти компьютера* (если *строка* *нуль-терминированная*, то её общий *размер увеличивается на один байт*).
