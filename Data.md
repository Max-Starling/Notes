...
- [Знаковые и беззнаковые целочисленные значения](знаковые-и-беззнаковые-целочисленные-значения)
- [Представление целочисленного значения в памяти](#представление-целочисленного-значения-в-памяти)
- [Размер значения целочисленного типа](#размер-значения-целочисленного-типа)

### О целочисленном типе

**Целочисленным типом** (англ. Integer, Integral data type, Int) называют *тип данных*, *каждое значение* которого *представляет* некоторое *число* из *множества целых чисел* `Z`, *изучаемого* в *математике*.

Например, *целочисленным значениями* являются: `17`, `-3`, `0`.

### Знаковые и беззнаковые целочисленные значения
*Целочисленные значения*, как и *целые числа*, могут быть *знаковыми* и *беззнаковыми*.

**Беззнаковым целочисленным значением** (англ. unsigned integer) называют *значение*, *представляющее* только *неотрицательные целые числа*.

*Беззнаковые целочисленные значения*: `0, 1, 2, 3, ...`.

**Знаковым целочисленным значением** (англ. signed integer) называют *значение*, *представляющее любое целое число* (*положительное*, *отрицательное* или *ноль*).

Например, *знаковыми целочисленными значениями* являются: `-37`, `8`, `0`.

<!--

## Представление целочисленного значения в памяти
...
*Математическое множество целых чисел* `Z`, вообще говоря, *бесконечно*.

*Целое число представляется* в *памяти компьютера* в виде *последовательности байт* (*бит*).
...
На *кодирование знака числа выделяется один бит*: `1` - *знак* `+`, `0` - *знак* `-`. Поэтому *одним байтом* (`1 байт` *занимает знак*, `7 бит` - само *число*) может быть *представлено знаковое число* от `0` до `127`.


*Беззнаковое целое число* в *промежутке от* `0` *до* `255` может быть *представлено одним байтом*.


В общем случае, 


Разберём на примере *байта*: 

Для представления целого числа воспользуемся *формулой количества информации* `n = log2(x)`


Тогда **количество информации `i` в одном символе** *алфавита* `A` вычисляется по *формуле*:  
**`i = log2(n)`**.

Представление целого числа аналогично представлению информации в одном символе:
`n = log2(x)`

`x = 2^n`

Всего *существует* `m = 2^n` *двоичных последовательностей длины* `n`, что *можно* легко *доказать комбинаторно*, используя *размещения с повторениями*.

Этими `m` *двоичными последовательностями* можно *представить беззнаковые целые числа* от `0` до `m - 1`. Таким образом, *последнее целое число* будет *равно* `m - 1 = 2^n - 1`.


Этими же `m` *последовательностями* можно

или *знаковые целые числа* от `-m - 1.
...
В зависимости от *количества байт*, используемых на *представление целочисленного значения*, *выделяют несколько типов целочисленных значений*. 
Подтип | Размер  | Диапазон допустимых значений
:--: | :--: | :--:
**Ниббл** (англ. **nibble**), **полуоктет** (англ. **semioctet**) | `4 бит` | *Беззнаковый*: *от* `0` *до* `15`, *знаковый*: *от* `-8` до `7`
**Байт** (англ. **byte**),  **октет** (англ. **octet**) | `8 бит` | *Беззнаковый*: *от* `0` *до* `255`, *знаковый*: *от* `-128` до `127`
**Полуслово** (англ. **halfword**), (англ. **short**) | `16 бит` | *Беззнаковый*: *от* `0` *до* `255`, *знаковый*: *от* `-127` до `127`
**Слово** (англ. **word**),  (англ. **long**) | `32 бит` | *Беззнаковый*: *от* `0` *до* `255`, *знаковый*: *от* `-127` до `127`
**Длинное слово** (англ. **longword**),  **двойное слово** (англ. **doubleword**), (англ. **quadword**, **qword**) | `64 бит` | *Беззнаковый*: *от* `0` *до* `255`, *знаковый*: *от* `-127` до `127`
(англ. **octaword**), (англ. **double quadword**) | `128 бит`| *Беззнаковый*: *от* `0` *до* `255`, *знаковый*: *от* `-127` до `127`
