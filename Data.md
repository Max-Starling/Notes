...
**Строковым типом** (англ. string) называют *тип данных*, *каждое значение* которого *представляет* собой *последовательность символов* некоторого *алфавита*. Эту *последовательность символов* называют **строкой** (англ. string), и она *представляет* собой *упорядоченный набор элементов с повторениями*.

### Обозначение строк

В *языках программирования значения строкового типа* (*строки*) обычно *заключаются* в *кавычки*:
...
Со временем пришли к тому, что `256` *символов не достаточно*, поэтому в *наши дни* на *один символ строки выделяют* `2-4 байта`. Использование `2 байт` позволяет 
