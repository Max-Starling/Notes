...
**Длинное слово** (англ. **longword**),  **двойное слово** (англ. **doubleword**) | `8 байт` (`64 бит`) | *Беззнаковый*: *от* `0` *до* `2^64 - 1`, *знаковый*: *от* `-2^63` до `2^63 - 1`
...
**Битовые** или **побитовые операции** (англ. bitwise operation) *предназначены* для работы с *последовательностями бит*.
...
* **Битовое отрицание**, **битовое “НЕ"** (англ. bitwise “NOT”), **дополнение** (англ. complement) *принимает двоичную последовательность* и производит *логическое отрицание каждого* её *бита*, то есть *ноль становится единицей*, а *единица становится нулём*. *Обозначение побитового отрицания* `x`: `NOT x`. 
...
* **Битовое “И”** (англ. bitwise “AND”) *принимает две двоичные последовательности* и производит *логическое “И”* для *каждой пары бит последовательностей*. *Обозначение битового “И”* для *последовательностей* `x` и `y`: `x AND y`.
...
* **Битовое “ИЛИ”** (англ. bitwise “OR”) *принимает две двоичные последовательности* и производит *логическое “ИЛИ”* для *каждой пары бит последовательностей*. *Обозначение битового “ИЛИ”* для *последовательностей* `x` и `y`: `x OR y`.
...
* **Битовое “ИЛИ-НЕ”**, **битовое исключающее “ИЛИ”** (англ. bitwise exclusive “OR”, bitwise XOR) *принимает две двоичные последовательности* и производит *логическое исключающее “ИЛИ”* для *каждой пары бит последовательностей*. *Обозначение битового исключающего “ИЛИ”* для *последовательностей* `x` и `y`: `x XOR y`.
...
<< 00011110 = 00111100
```
* При **арифметическом сдвиге** (англ. arithmetic shift) *сохраняется знак числа*, который *хранится* в *старшем бите*, а значит *сохраняется значение старшего бита*. В *остальном арифметический сдвиг аналогичен логическому*.
```
>> 10001111 = 11000111
>> 11000111 = 11100011
