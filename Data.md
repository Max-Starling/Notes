...
*Преобразование* (*отображение*) *символов алфавита* `A` в *символы алфавита* `B` называют **кодом** (англ. code), а *если алфавит* `B` *состоит лишь* из *нулей и единиц*, то есть `B = { 0, 1 }`, то *преобразование* называют **бинарным кодом** (англ. binary code). 

*Математически код* можно *представить* как *функцию* (*отображение*) `f(x) = y`, где `x` - *упорядоченный набор (кортеж) символов алфавита* `A`, а `y` - *упорядоченный набор (кортеж) символов алфавита* `B`. Тогда *пример выше* можно *записать* как `f('a') = 01000001`.




<!--
### Однобайтовые кодировки и Unicode


Ранее выделяли `1 байт = 8 бит` памяти на каждый символ сообщения. Это позволяло использовать наборы символов, содержащие до 256 символов включительно.


Как было показано ранее, один байт может быть представлен 2^8 = 256 различными последовательностями из восьми бит. Таким образом, выделение одного байта на один символ позволяет использовать алфавит, содержащий до 256 символов.
...
*Раньше* было *принято* использовать *однобайтовые кодировки*, то есть такие *наборы символов*, *каждый символ* которых *занимает один байт*. 
...
Наиболее ярким *примером однобайтовой кодировки* выступает *кодировка ANSII*, являющаяся *стандартом* в *языке C* и часто *встречающаяся* при работе с языком *C++*.
...
*Однобайтовые кодировки* обладали *преимуществом* в своей *легковестности*, однако являлись сильно *ограниченными*. *Однобайтовая кодировка могла вместить лишь несколько языков*. 

Чаще всего для каждого разговорного языка использовалась своя кодировка, также включающая набор основных специальных символов. Это может быть удобно в рамках одной локальной системе, но совершенно не удобно, когда много таких систем взаимодействуют друг с другом, а именно это и подразумевает интернет. Так, например, текстовый файл, напечатанный на одном компьютере, невозможно было прочитать на другом при отсутствии требуемой кодировки.
