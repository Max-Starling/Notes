...
- [Кодирование и набор символов (кодировка)](#кодирование-и-набор-символов-кодировка)
...
### Сообщение
Самым распространённым *способом передачи информации* является *сообщение*.
...
## Кодирование и набор символов (кодировка)

<!--
**Кодированием** (англ. encoding) называют *процесс преобразования информации* (*последовательности символов*) в *последовательность * (*данные*) для дальнейшей *обработки*.
-->
...
Обычно *каждому символу* в *наборе символов* ставится в *соответствие* некоторое *целое число*, *аналогично* тому, как *каждому элементу массива* ставится в *соответствие* свой *индекс*. Это *целое число* часто *записывают* в *двоичной* или *шестнадцатиричной системах счисления*.

Например, пусть *символу* `'b'` в некотором *наборе символов* `A` *соответствует число* `65`. *Значение* этого *числа* в *2 с/с*: `01000001`, в *16 с/с*: `41`.


Само *преобразование* (*отображение*) *символов алфавита* `A` в *символы алфавита* `B` называют **кодом** (англ. code), а *если алфавит* `B` *состоит лишь* из *нулей и единиц*, то есть `B = { 0, 1 }`, *преобразование* называют **бинарным кодом** (англ. binary code). 
...
В наши дни самым распространённым
...
1) **Логический тип**. Содержит *истину* или *ложь*. Например, `true` или `false`, `1` или `0`.  
