...
**Набором символов** (англ. character set, charset) называют некоторый *конечный алфавит*, *символы* которого *могут* быть *использованы* во многих *других алфавитах* (например, в *алфавитах разговорных языков*).

Например, некоторый *набор символов* может *содержать символы латинского алфавита*, а *эти символы используются* в *английском*, *немецком*, *французском*, *итальянском* и других алфавитах.

**Кодировкой символов** (англ. character encoding) или просто **кодировкой** (англ. encoding) называют *алгоритм* (*функцию*), который *каждый символ* из *набора символов* *преобразует* в *последовательность символов* некоторого *другого алфавита*.

**Закодированным набором символов** (англ. coded character set) называют *таблицу*, которая *каждому символу* *набора символов* *ставит* в *соответствие последовательность из одного или нескольких символов* некоторого *другого алфавита*. 

<!--
*Раньше* с *этим определением* было принято *использовать понятие* “**кодировка**” (англ. encoding). Хотя эти два *понятия* часто используют как *синонимы*, *смысл* у них *отличается*.
-->

Часто *закодированный набор символов* называют *кодировкой*.
...
Например, пусть в некотором *закодированном наборе символов* `A` *символу* `'#'` *соответствует число* `124`, а *символу* `'$'` *соответствует число* `255`. *Значение* *первого числа* в *2 с/с равно* `01111100`, а *второго* - `11111111`; в *16 с/с значение первого числа равно* `7c`, а *второго* - `ff`.
...
Наиболее ярким *примером семибитной кодировки* выступает **кодировка ANSII** (англ. American Standard Code for Information Interchange), а *восьмибитной кодировки* - *расширенная кодировка ANSII* (англ. Extended ANSII), являющаяся *стандартом* в *языке C* и часто *встречающаяся* при работе с языком *C++*.

Кодировка ANSII содержит символы латинского алфавита

Каждому символу кодировки ANSII ставится в соответствие целое число от 0 до 127, которое может быть представлено 7-битной строкой.

Символ | Код символа | Бинарный код символа
:--: | :--: | :--:
... | ... | ...
0 | 48 | 
1 | 49 |
