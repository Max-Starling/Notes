...
*Нормализация позволяет избежать избыточности данных*, то есть *исключить дублирование данных*. *Дублирование данных* в свою очередь может стать причиной ошибок при изменении данных (ведь нужно всего изменять не только сами данные, ну и все их дубликаты).
...
Когда *отношений слишком много* и они *тесно связаны* друг с другом, *группировка данных* может *занимать продолжительное время* (скажем, сделать объединение `INNER JOIN` десяти таблиц), что плохо сказывается на производительности системы и на её работе с пользователем в целом. 

По этой причине данные иногда умышленно хранят в денормализованном виде, что хоть и усложняет работу с такими данными и увеличивает размер базы данных в целом, но зато существенно снижает время отклика базы данных.
