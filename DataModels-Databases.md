...
*Самой простой базой данных* может *послужить обычный текстовый файл*, однако *искать* и *изменять данные* в такой *"базе"* было бы как минимум *неудобно*.
...
* **Иерархическая** (Hierarchical). Данные организованы в виде иерархии.
...
* **Согласованность, консистентность** (англ. **C**onsistency). Каждая успешная транзакция фиксирует только допустимый для системы результат. База данных должна быть согласована до и после транзакции, во время проведения транзакции согласованность не требуется.
* **Изолированность** (англ. **I**solation). Способность базы данных гарантировать, что *параллельно выполняющиеся транзакции не мешают друг другу*. Во время выполнения транзакции другие параллельные транзакции не должны оказывать влияния на её результат. Требование "дорогое", поэтому в реальных базах транзакции изолируются не полностью: создаются *уровни изолированности*.
...
Уровни:
* **Чтение неподтверждённых данных** (англ. `Read Uncommitted`) - можно читать неподтверждённые данные (грязное чтение). Грязные чтения, неповторяющиеся чтения, фантомные чтения возможны.
...
* **Повторяемое чтение** (англ. `Repeatable Read`) - все данные, прочитанные в транзакции, будут *одинаковыми при повторном чтении*. Грязные и неповторяющиеся чтения невозможны. Возможны фантомные чтения.
...
Наиболее распространённый тип индекса (чаще всего используется по умолчанию). Организован в виде сбалансированного дерева поиска.
Когда использовать:
...
Если у таблицы 5 индексов, то при добавлении новой строки база данных должна обновить все 5 индексов, что занимает больше времени, чем без индексов или с одним индексом.
...
## Шардинг 
**Шардинг** (англ. sharding) — приём масштабирования, который позволяет распределять данные между разными физическими серверами. Процесс шардинга предполагает разнесение данных между отдельными шардами на основе некого ключа шардинга. Связанные одинаковым значением ключа шардинга сущности группируются в набор данных по заданному ключу, а этот набор хранится в пределах одного физического шарда. Это существенно облегчает обработку данных.
...
Партиционирование таблиц делит весь объём операций по обработке данных на несколько независимых и параллельно выполняющихся потоков, что существенно ускоряет работу СУБД.
...
Вообще говоря, *основными типами данных* в *программировании* являются *числовой* (`7`, `3.14`), *строковый* (`"развитие"`, `'engineer'`) и *логический* (`истина`, `ложь`).
...
Если мы указываем, что *значение принадлежит какому-то типу*, то *другому типу оно* уже *принадлежать не может*.
...
В *реляционной модели данных* *все значения одного атрибута принадлежат одному конкретному типу данных*. Например, *нельзя*, чтобы *атрибут имел числовые* и *строковые значения одновременно*.
...
Таким образом, *заголовок n-арного отношения* имеет вид: `H = { (a1, t1), (a2, t2), ..., (an, tn) }`.
...
Пусть имеется `n` атрибутов `a1, a2, ..., an`. Тогда **тело** `B` (англ. body) **отношения** `R` - это *множество кортежей* `k = ((a1, v1), (a2, v2), ..., (an, vn))`: `B = { k1, k2, ..., km }`. 
...
Например, можно по *номеру паспорта определить* (*идентифицировать*) *человека*, по *IP-адресу* - *физический компьютер*, по *координатам* - *точку* на карте, по *штрих-коду* - *товар* в магазине, по *QR-коду* - *ссылку*, по *ссылке* - *сайт* и так далее.
...
**Условие минимальности** (*несократимости*) *требует*, чтобы *среди атрибутов потенциального ключа отсутствовало меньшее подмножество* из `k <= m` *атрибутов*, которое *удовлетворяет условию уникальности*. 
...
На *примере ниже* представлено *отношение* с *двумя потенциальными ключами*, *каждый* из которых *состоит* из *одного элемента*: `{ Номер телефона }` и `{ Email }`.
...
1) **Удобство использования**. Чаще всего *выбирается потенциальный ключ* с *наименьшим физическим размером* (*занимает меньше памяти* на *компьютере*) или *содержит наименьшее количество атрибутов*. 
...
Как и в случае с *потенциальными ключами*, *естественных ключей* у *отношения* может быть *несколько*.

*Примеры* *естественных ключей* всё *те же*, что и *примеры* *потенциальных ключей*: *человека* можно *идентифицировать* по *серии* и *номеру паспорта*, *пользователя* - по *email*, *дом* - по *адресу* и так далее. *Всё* это - *естественные ключи*, *взятые* из *предметных областей* (*доменов*) соответствующих *объектов*.
...
В *отличие* от *естественного ключа*, *суррогатный ключ никак не зависит* от *свойств* *реального объекта*, а следовательно *и от атрибутов*. *Суррогатный ключ генерируется системой* по *заданному алгоритму*, поэтому *не имеет смысла вне системы*.

Чаще всего *суррогатный ключ представляет собой* некоторый *хэш* (*произвольно сгенерированная строка*) или *целое число*.
...
2) *Каждое значение* внешнего ключа `FK` в *некотором кортеже* отношения `S` *совпадает* со *значением* потенциального ключа `CK` в *некотором кортеже* отношения `R`. Иначе говоря, *множество значений* ключа `FK` *содержится* (является *подмножеством*) в *множестве значений* ключа `CK`.
...
*Связи между отношениями похожи* по *смыслу* на *бинарные отношения из теории множеств*. *Связи передают* то, как *объекты соотносятся* (связаны) *друг с другом*. В *терминах реляционной модели данных*: *связь устанавливает соотношение* (соответствие) *между кортежами двух отношений*, причём *не обязательно различных*.

*Связи между* двумя *отношениями устанавливаются* при помощи *внешних ключей*.
...
*Первичные ключи могут изменяться*. Простой пример: *пользователь* хочет *сменить email* или *номер телефона*. Если *существует связь*, *использующая* подобный *потенциальный ключ* `CK`, то *ссылающийся* на него *внешний ключ* `FK → CK` также *должен измениться*.
...
Можно придумать *бесконечно много реляционных операций*, но *абсолютное большинство* из них будут *выражаться через другие* реляционные операции, то есть *будут являться результатом последовательного применения нескольких* более простых *реляционных операций*.
...
**Пересечение отношений `R` и `S`** - *бинарная операция*, *требующая совпадения заголовков* отношений `R` и `S`, *результатом* которой является *отношение* с *тем же заголовком* и *телом*, *содержащим только* те *кортежи*, которые *содержатся в обоих отношениях*. Иначе говоря, *каждый кортеж* отношения-результата *содержится и в* отношении `R`, *и в* отношении `S`.
...
Другими словами, в *результат деления отношения* `R` *на отношение* `S` *попадают такие* кортежи `x = (x1, x2, ... xn)` *делимого* `R`, c которыми *каждый кортеж* `y = (y1, y2, …, ym)` *делителя* `S` (то есть *все кортежи* отношения `S` *без исключения*) *состоит в упорядоченной паре* `(x, y)`, или, что *то же самое*, в кортеже `(x1, x2, ..., xn, y1, y2, ..., ym)`, *принадлежащем отношению `R`*.
...
*Обозначение переименования атрибутов* `{ a1, a2, ..., am }` отношения `R` *на новые имена* `{ b1, b2, ..., bm }`: `R RENAME a1, a2, …, am AS b1, b2, …, bm`.
...
magic | 240 | funnyrabbit | 180 | 0.5 | бирюзовый
woodruf | 180 | funnyrabbit | 180 | 0.5 | бирюзовый

Итак, пользователь `magic` по *своему запросу* может *купить предмет* у *пользователя* `evergreen` или у `funnyrabbit`, а *пользователь* `woodruf` - *только* у `funnyrabbit`. *Пользователь* `redhot` *остался без потенциальных покупателей*, поскольку *установил слишком высокую цену*.
...
5) **Дистрибутивность**  
 - *проекции относительно пересечения, объединения, разности и декартова произведения*: `(R ◇ S)[a] = R[a] ◇ S[a], ◇ ∈ { INTERSECT, UNION, MINUS, TIMES }`.
 - *выборки относительно объединения и декартова произведения*: `(R ◇ S) WHERE c = (R WHERE c) ◇ (S WHERE c), ◇ ∈ { UNION, TIMES }`.  
...
Важно отметить, что *декартово произведение отношений коммутативно* и *ассоциативно в отличие* от *декартова произведения множеств*. Это *связано* с тем, что *кортежи отношения не должны быть упорядоченными*.
...
Говоря *другими словами*, если *два кортежа отношения* `R` *совпадают* по *значениям* в *подмножестве атрибутов* `A`, то эти *кортежи* также *должны совпадать* по *значениям* и в *подмножестве атрибутов* `B`.
...
*Понятие нормализации ввёл Эдгар Кодд* как *часть* своей *реляционной модели*.
...
По этой причине данные иногда умышленно хранят в денормализованном виде, когда одно отношение представляет собой композицию нескольких отношений.
...
#### Пользователь (хранит идентификаторы публикаций, чтобы публикации было проще найти на странице пользователя)
ID | Никнейм | Почта | Возраст | Публикации
:--: | :--: | :--: | :--: | :--:
`U1` | @max.starling | max.starling@email | 23 | [`N1`, `N2`]
`U2` | @ray.gray | ray.gray@email | 27 | [`N3`]
...
Таким образом, *значение атрибута не может представлять собой набор значений* (массив, множество, таблицу и так далее), если *отношение находится* в *1НФ*.
...
Резидентные базы данных могут использовать и дисковую память, но в первую очередь используют оперативную память. Это главное отличие от традиционных СУБД (например, MySQL) и современными базами данных (NoSQL), которые в первую очередь используют диск.
...
При нативной реализации такого запроса на SQL мы сначала возьмём пользователей
```sql
SELECT * FROM users
```
...
```
Итого у нас лишь один запрос к базе вместо `n + 1`.
...
Оставляем задачу с пользователями и постами, имеем 1 запрос для пользователей и `n` запросов постов для каждого из `n` пользователей.
```js
...
Учитывайте **селективность** (индексируйте поля с высокой уникальностью).
...
## Выбирайте только нужные поля таблицы и не используйте `SELECT *`
```sql
# плохо
SELECT *
# хорошо
