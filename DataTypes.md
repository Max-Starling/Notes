...
**Экспоненциальной формой записи числа `N`** называют *представления числа* в виде *произведения некоторого* другого *числа* `m`, называемого **мантиссой** (англ. mantissa, significand) и *показательной функции* `b^e`, в которой *число* `b` называют **основанием** (англ. base), а *число* `e` - **экспонентой** (англ. exponent): **`N = m • b^e `**.

Чаще всего в качестве основания берут число `10`.
