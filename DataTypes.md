...
*Ссылку* можно использовать как *псевдоним*: *создание ссылки позволяет обратиться* к *старому значению* по *новому* (еще одному) *имени*.
...
*Параметры функции* представляют собой *локальные переменные*, *доступные лишь внутри* этой *функции* и *ссылающиеся* на *значения аргументов*, *передаваемых* в эту *функцию*.
...
Использование *ссылки* в *качестве параметра* позволяет *изменять значение аргумента примитивного типа прямо* в самой *функции*.
```cpp
/* функция, увеличивающая значение аргумента на 1 */
void increment(int &x) {
  return ++x;
