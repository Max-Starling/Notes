...
*Несмотря* на *похожесть формул*, *точность* `p` и *точность вычислений* `p` имеют *существенное различие* в своём *предназначении*: `a` *вычисляется* для *проверки точности промежуточного* или *конечного результата*, а `p` *задаётся* для *ограничения области допустимых значений*.
