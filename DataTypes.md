...
Говорят, что *ссылка* **ссылается** (англ. refers) на *данные*.

*Получение данных* (*доступ к данным*) по *ссылке* называют **разыменованием ссылки** (англ. dereferencing).

*Ссылки* напоминают *псевдонимы*: они *позволяют обратиться* к *одной* и *той же переменной* по *разным именам*.

*Вывод значения ссылки* приводит к *выводу* тех *данных*, на которые она *ссылается*.
...
*Ниже* представлен *пример передачи объекта* по *ссылке* на *языке JavaScript*, в котором *применяется лишь внутренняя реализация ссылки*.
```js
