...
В *основу логического типа данных* легла *математическая логика*.
...
*Реализовать* сам *бит* в *компьютере* достаточно *просто*: например, *подача напряжения* по *проводу* - это `1` (*истина*), *отсутствие напряжения* на *проводе* - `0` (*ложь*).
...
* *Получение индекса символа* в строке. Например, `("Notes").find("t") --> 2`.
...
Например, `"music" > "museum"`, поскольку *код символа* `'i'` *больше кода символа* `'e'`. Ниже представлена *таблица* для *наглядности сравнения*. *Коды символов* взяты из *кодировки ASCII*.
...
*Значение символьного типа занимает столько памяти*, *сколько* должен занимать *символ соответствующей кодировки*. Например, для *ANSI* это всегда `1 байт`, для *кодировок семейства UTF размер варьируется от* `1` *до* `4 байт`.
...
- [Размер целого, короткое и длинное целые](#размер-целого-короткое-и-длинное-целые)
...
**Целое число со знаком** (англ. signed integer) *представляет любое целое число* (*положительное*, *отрицательное* или *ноль*). Например, `-37`, `8`, `0`.
...
* `0` - *знак* `+`, *соответствует неотрицательному числу*,
...
*Число* `- (m / 2) = - 2^(n - 1)` называют **минимальным целым числом со знаком** (*нижняя граница множества*), а *число* `(m / 2) - 1 = 2^(n - 1) - 1` называют **максимальным целым числом со знаком** (*верхняя граница множества*).

Например, *одним байтом* (`n = 8`) могут быть *представлены целые числа со знаком* от `-2^7 = -128` *до* `2^7 - 1 = 127`.

#### Прямой код

Самый *очевидный способ закодировать знак* - **прямой код** (англ. `sign-magnitude`): *старший бит хранит знак*, а *оставшиеся* `n - 1` *бит* - *модуль числа*.

*На практике прямой код не используется* по двум причинам:  
1) *Появляются два нуля*: `00000000` (`+0`) и `10000000` (`-0`). Из-за этого *теряется одно значение* и *усложняется сравнение чисел*.  
2) *Сложение чисел с разными знаками требует отдельного алгоритма*: *обычное побитовое сложение даёт неверный результат*.

#### Дополнительный код

В *современных компьютерах целые числа со знаком кодируются* **дополнительным кодом** (англ. `two's complement`).

*Неотрицательные числа* записываются так же, как и *беззнаковые*. Чтобы *получить код отрицательного числа* `-x`, необходимо:  
1) записать *двоичный код числа* `x`,  
2) *инвертировать все биты* (заменить `0` на `1` и наоборот),  
3) *прибавить единицу*.

Например, *закодируем* `-2` при `n = 8`: число `2` - это `00000010`, после *инверсии* - `11111101`, после *прибавления единицы* - `11111110`.

*Преимущества дополнительного кода*:
* *Ноль только один*: `00000000`.
* *Сложение* и *вычитание* выполняются *одним и тем же алгоритмом* вне зависимости от *знаков операндов*.
* *Освобождается лишняя комбинация* `10000000`, которая *отдаётся числу* `-128`. Именно поэтому *диапазон несимметричен*: от `-128` до `127`.

В *таблице ниже целые числа со знаком закодированы одним байтом* в *дополнительном коде*.
...
:--: | :--: | :--: | :--:
**C** | не менее `16 бит` | не менее `16 бит` | не менее `32 бит`
**C++** | не менее `16 бит` | не менее `16 бит` | не менее `32 бит`
**C#** | `2 байта` | `4 байта` | `8 байт`
**Java** | `2 байта` | `4 байта` | `8 байт`

В *C* и *C++* *стандарт задаёт* только *минимальные размеры* ([C11, черновик N1570, §5.2.4.2.1](https://www.open-std.org/jtc1/sc22/wg14/www/docs/n1570.pdf); [C++, таблица «Minimum width» в разделе basic.fundamental](https://eel.is/c++draft/basic.fundamental)), а *точный размер* выбирает *компилятор* под *конкретную платформу*. Например, *MSVC* в *Windows* отводит под `int` и `long` по `4 байта` ([документация Microsoft](https://learn.microsoft.com/en-us/cpp/cpp/data-type-ranges?view=msvc-170)), а *Apple clang* в *macOS* на процессорах *Apple* - `4 байта` под `int` и `8 байт` под `long`.

В *C#* и *Java* *размеры фиксированы спецификацией языка* ([C#](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/builtin-types/integral-numeric-types), [Java, JLS §4.2](https://docs.oracle.com/javase/specs/jls/se21/html/jls-4.html)).
...
* **Декремент** (англ. **dec**rement) - *уменьшение значения на единицу*. *Обозначение декремента числа* `x`: `x--` или `--x`. Например, `--4 = 3`.
...
* **Вычитание** (англ. **sub**traction). *Обозначение вычитания числа* `y` из *числа* `x`: `x - y`. Например, `7 - 6 = 1`.
...
* **Битовое исключающее “ИЛИ”** (англ. bitwise exclusive “OR”, bitwise XOR) *принимает две двоичные последовательности* и производит *логическое исключающее “ИЛИ”* над *каждой парой бит*, которые *стоят* на *одинаковых позициях* в *последовательностях*. *Обозначение битового исключающего “ИЛИ”* для *последовательностей* `x` и `y`: `x XOR y`.
...
* **Битовый сдвиг влево** (англ. left bitwise shift), *сдвиг* от *младшего бита* к *старшему*, подразумевает *смещение* (*перестановку*) *каждого* из *значений средних бит* на *1 бит влево*.
* **Битовый сдвиг вправо** (англ. right bitwise shift), *сдвиг* от *старшего бита* к *младшему*, подразумевает *смещение* *каждого* из *значений средних бит* на *1 бит вправо*.
...
В *компьютере* для записи *очень больших* и *очень маленьких чисел* также используется *научная (нормализованная) форма записи чисел* в виде: **`n = mEe`**, где **`E`** означает `10^`.
...
`7` | `7 • 10^0` | `7.0`
`-322` | `-3.22 • 10^2` | `-322.0`
`19.84` | `1.984 • 10^1` | `19.84`
`0.000000001` | `1 • 10^(-9)` | `1E-09`
`-0.0000000000177` | `-1.77 • 10^(-11)` | `-1.77E-11`
`300000000` | `3 • 10^8` | `3E+08`

Часто *языки программирования* предоставляют *несколько типов данных*, *каждый* из которых используется для *чисел с плавающей точкой*. *Обычно* такими являются *типы данных* `Float` и `Double`.
...
*Сложение* и *разность чисел с плавающей точкой требуют совпадения порядков* чисел: **`e = e1 = e2`**. Если *порядки не совпадают*, то *необходимо перенести точку* в одной из *мантисс* `m1` и `m2` так, чтобы *порядки* `e1` и `e2` *совпали*.
...
Из *примеров выше следует*, что *ссылку* можно *использовать как псевдоним*: *создание ссылки позволяет обратиться* к некоторому *старому значению* по *новому* (ещё одному) *имени*.
...
Например, в *языке JavaScript*, как и во многих других языках, *нет возможности явно* (*напрямую*) *использовать ссылки*, тем не менее *передача* любого *объекта* *осуществляется* по *ссылке* (*внутренняя реализация*, *неявное использование ссылки*).
```js
...
В *отличие* от *ссылки*, *указатель может быть нулевым*, то есть *может не указывать* ни на один *объект*.
```cpp
int *ref = NULL;
```
...
В *отличие от математики*, где *сравнение представляет* собой *отношение*, в *программировании сравнение* является *операцией*, то есть оно *обязательно возвращает результат* (обычно *логического типа*).
-->
