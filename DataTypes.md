...
- [Ссылка и указатель](#ссылка-и-указатель)
...
**Ссылкой** (англ. reference) в *программировании* называют *объект*, который *указывает* на *определённые данные*, но *не хранит* их.

Говорят, что *ссылка* **ссылается** (англ. refers) на некоторые *данные*.

*Получение данных* (*доступ к данным*) по *ссылке* называют **разыменованием ссылки** (англ. dereferencing).

*Ссылки* похожи на *псевдонимы*: они *позволяют обратиться* к *одному* и *тому же значению* с помощью *различных имён*.
...
Поскольку *значением ссылки* являются те *данные*, на *которые* она *ссылается*, *ссылка не может быть пустой*, то есть она *не может не иметь значения*.
```cpp
int &ref; // ошибка
```

В *большинстве языков программирования ссылки* являются лишь *частью внутренней реализации* и *не доступны* для *использования*.

Например, в *языке JavaScript*, как и в многих других языках, *нет возможности использовать ссылки напрямую*, но при этом *передача объекта осуществляется* по *ссылке* (*внутренняя реализация*).
```js
