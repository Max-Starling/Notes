...
### Число сообщений длины k из символов алфавита мощности n

Рекомендуется прочитать: [Размещения с повторениями (комбинаторика)]().

*Алфавитом* называют *множество символов*.

*Сообщение* представляет собой *упорядоченный набор элементов* (*символов*) *с повторениями*, *составленный* из *элементов* некоторого *множества* (*алфавита*) `A`.

Таким образом, **число сообщений длины `k` из символов алфавита мощности `n`** *соответствует* **числу размещений с повторениями из `n` по `k`**, а именно **`A(n, k) = n^k`**.
