...
Примеры *множеств*: *множество женских имён*: `N = { Ася, Сара, Рози }`, *множество ролей*: `R = { user, admin }`, *множество дат*: `D = { 2019, 2020 }`.
...
Примеры кортежей: *кортеж из элементов множеств* `N`, `R`, `D` соответственно: `(Ася, admin, 2019)`, *координаты точки* на плоскости: `(1, 1)`, *набор аргументов функции* `f(x1, x2, x3)`.
...
Например, декартово произведение `N × D = { (Ася, 2019), (Ася, 2020), (Сара, 2019), (Сара, 2020), (Рози, 2019), (Рози, 2020) }`, а декартово произведение `N × R × D` состоит из `3 • 2 • 2 = 12` кортежей. по типу `(Ася, admin, 2019)`.
...
Понятие отношения в реляционной модели данных отличается от понятия отношения в теории множеств лишь тем, что множествами `X1, X2, ..., Xn` выступаювыступают типы данных `T1, T2, ..., Tn` (также являющиеся множествами по определению).

Пусть *имеются типы данных* `T1, T2, ..., Tn` (*не обязательно различные*), тогда **n-арным отношением** `R` называют *подмножество декартового произведения* `T1 × T2 × ... × Tn`. 

Грубо говоря, отношение - это некоторое множество (набор) кортежей с одинаковой структурой (схемой), то есть кортежей `(t1, t2, ..., tn)`, где `t1, t2, ..., tn` принадлежат множествам `T1, T2, ..., Tn` соответственно.
...
**Заголовком** (*схемой*) `H` (англ. header) **отношения** `R` называют *множество упорядоченных пар* `(a, t)`, где `a` - *название атрибута*, `t` - *название типа данных* (не сам тип данных, являющийся множеством), соответствующего атрибуту.

Итак, `H = { (a1, t2), (a2, t3), ..., (an, tn) }` - *заголовок n-арного отношения*.

Пример заголовка для отношения между множествами `N`, `R`, `D`: `H = { (Имя, string), (Роль, string), (Дата, date) }`.
...
*Отношения* нередко *называют таблицами*, но это *не* совсем *правильно*. *Таблица* с её *строками* и *столбцами* - лишь *визуализация*: эти понятия *не отражают сути понятия отношения* и его *свойств*.
