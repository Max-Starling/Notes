...
Пусть имеется `n` атрибутов `a1, a2, ..., an`. Тогда **тело** `B` **отношения** `R` - это *множество кортежей* вида: `K = ((a1, v1), (a2, v2), ..., (an, vn))`. *Количество упорядоченных пар должно совпадать* с *количеством атрибутов*.
