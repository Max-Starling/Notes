...
**Объединения отношений `R` и `S`** - бинарная операция, требующая совпадения заголовков отношений `R` и `S`, результатом которой является отношение с тем же заголовком и телом, содержащим кортежи обоих отношений. Если отношения имеют одинаковые кортежи, то в результат попадает только один из них.
