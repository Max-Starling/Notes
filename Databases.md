...
*Суррогатный ключ не зависит* от *свойств реального объекта*, он *генерируется системой* и, следовательно, *зависит от системы*. В этом его *главное различие* с *естественным ключом*.

У *отношения* может быть *несколько суррогатных ключей*.

Самые распространённые *способы генерации суррогатного ключа*:  
1) **Universally Unique Identifier** (*UUID*). *Пример UUID*: `9fc2e9ad-fae7-4f25-ae48-3f45284f2299`.
2) **Globally Unique Identifier** (*GUID*). *Пример GUID*: `75f067fa-b95f-405c-a1bd-29164461a65f`.
3) **Object Identifier** (*ObjectID*). *Пример ObjectID*: `ObjectId("507f1f77bcf86cd899439017")`.
4) `AUTO_INCREMENT` (MySQL), `AUTOINCREMENT` (SQLite) `SEQUENCE` (SQL Server, Oracle).

<!--
*Примеры суррогатных ключей*: *UUID*, *GUID*, *ObjectID*, *порядковые номера объектов* (*автоинкрементные целочисленные значения*), *даты создания объектов*.
-->
...
В *отличии* от *естественного ключа*, *суррогатный ключ не имеет смысла вне системы*.
...
2) **Компактность**. Суррогатный ключ всегда состоит только из одного значения, размен которого фиксирован и обычно невелик (более того, размер можно контролировать), что уменьшает затрачиваемую память и увеличивает скорость поиска объектов по ключу.
3) **Единообразие**. Если все ключи в системе создаются по одному алгоритму (например, везде используется UUID или ObjectID), то логика обработки данных упрощается и становится предсказуемой для разработчика в любом месте программы.
