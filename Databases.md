...
Самыми *распространёнными примерами операций* являются *арифметические операции*: *бинарная операция “сложение”* (`x1 + x2 = y`), *бинарная операция “возведение в степень”* (`x1 ^ x2 = y`), *унарная операция “факториал”* (`x! = y`) и так далее.
...
Грубо говоря, *отношение* - это некоторое *множество* (набор) *кортежей*, имеющих *одинаковую схему*, то есть *кортежей* `(t1, t2, ..., tn)`, *элементы* `t1, t2, ..., tn` которых *принадлежат типам данных* `T1, T2, ..., Tn` *соответственно*.

*Отношение состоит* из *заголовка* `H` и *тела* `B`. Фактически, *отношение* `R` можно *рассматривать* как *упорядоченную пару* `(H, B)`.


### Заголовок отношения

*Заголовок отношения состоит* из *атрибутов* и *соответствующих* им *типов данных*.
...
**Заголовки** двух **отношений совпадают**, если *совпадают* их *количество атрибутов, все названия атрибутов* и их *типы данных*. 
...
<!--
**Значение атрибута** (англ. attribute value) - название атрибута, связанное со значением `v` типа `T`, соответствующим 
...
Пусть имеется `n` атрибутов `a1, a2, ..., an`. Тогда **тело** `B` (англ. body) **отношения** `R` - это *множество кортежей* вида: `K = ((a1, v1), (a2, v2), ..., (an, vn))`. *Количество упорядоченных пар* в *кортеже* `K` *должно совпадать* со *степенью отношения* `n`.
...
*Отношения* нередко *называют таблицами*, но это *не* совсем *правильно*. *Таблица* лишь *отображает составные части отношения* в удобной форме, но *не отражают сути понятия отношения* и его *свойства*. Например, *понятие таблицы не подразумевает уникальности строк* в то время, как в *отношении требуется уникальность кортежей*.
...
Можно придумать *бесконечно много реляционных операций*, но абсолютное *большинство* из них будет *выражаться через другие*, более *простые реляционные операции*.
...
Операции *проекции*, *выборки*, *соединения* и *деления* называют **специальными операциями**: они имеют смысл лишь при работе с отношениями.
...
*Ниже* представлен *пример декартова произведения отношений* `R` и `S` с использованием *таблиц для наглядности*.
...
*Количество атрибутов* в *отношении* `T = R TIMES S`: `n(R) + n(S) = 2 + 2 = 4`, *количество кортежей*: `m(R) • m(S) = 3 • 2 = 6`.

#### Отношение `T(Имя, Фамилия, Профессия, Дата)`
...
*Ниже* представлен *пример проекции отношения* `R` с использованием *таблиц для наглядности*.
...
**Выборка из** (*ограничение*) *отношения* `R` - *унарная операция* над отношением `R`, *результатом* которой является *отношение* с *тем же заголовком*, что и у `R`, *содержащее* в своём теле те *кортежи* из `R`, которые *удовлетворяют* некоторому *условию* `c`.
...
*Ниже* представлен *пример выборки из отношения* `R` с использованием *таблиц для наглядности*.
...
Одним из *самых простых соединений* является то *соединение*, *в условии* которого используются *константа* (например, число или строка) и *один атрибут* одного из отношений-операндов соединения. Такое *соединение отношений* `R` и `S` представлено на *примере ниже с использованием *таблиц для наглядности*.
...
В *отличие* от *остальных операций*, *предложенных*, *деление отношений* не обрело широкую популярность и *используется крайне редко*. *Такой же результат* можно получить *комбинацией* операций *выборки и проекции* - такой подход будет *гораздо гибче*.
...
**Делением отношения `R` на отношение `S`** называют *бинарную операцию* над отношениями `R` и `S`, *результатом* которой является *отношение* `T` с *заголовком* `(x1, x2, …, xn)` и *телом*, *содержащим* множество таких *кортежей* `(x1, x2, …, xn)`, таких, что *для каждого кортежа* `(y1, y2, …, ym)` отношения `S` *существует кортеж* `(x1, x2, …, xn, y1, y2, …, ym)`. В этом случае *отношение* `R` *называют* **делимым**, *отношение* `S` - **делителем**, *отношение* `T` - **частным**.


*Обозначение деления отношения* `R` *на* `S`: `R DIVIDEBY S`.

<!--
Пример с языками; английский, русский, итальянский, имена
-->

Чем это *похоже на деление чисел*? `7 ÷ 3 = 3 + 3 + 1 = 2 целых и одна третья` (`2` - *количество раз*, которые *тройка поместилась* в число `7` *целиком*). В *примере выше* мы *возвращаем такие* кортежи `x1, x2, ... xn` *делимого* `R`, c которыми *все (целиком)* кортежи `(y1, y2, …, ym)` *делителя* `S` *состоят в паре*.
...
*Общим атрибутом отношений* `G` и `P` является только `Код товара`.
