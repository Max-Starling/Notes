...
**Выборка из** (*ограничение*) *отношения* `R` - *унарная операция* над отношением `R`, результатом которой является отношение с тем же заголовком, что и у `R`, содержащее в своём теле кортежи из `R`, удовлетворяющие некоторому условию `c`.
