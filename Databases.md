...
Пусть имеется `n` атрибутов `a1, a2, ..., an`. Тогда **тело** `B` **отношения** `R` - это *множество кортежей* `K = ((a1, v1), (a2, v2), ..., (an, vn))`.

То есть `B = { K1, K2, ..., Km }`, где `m` - *число кортежей*, которое называется **кардинальным числом отношения** (*мощностью отношения*).
