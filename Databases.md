...
**Данные** (англ. data) - это *форма представления информации*. *Информация* - это *смысл*, который *содержится* в *данных*. В этом смысле, *информация* является *абстракцией*, а *данные* являются её *реализацией*.

Часто понятия “*данные*” и “*информация*” используют как *синонимы*.

В *теории информации* *информацией* называют *упорядоченную последовательность символов* некоторого *алфавита*.
...
*Основной минимальной единицей измерения информации* является **бит** (англ. **bi**nary digi**t**), называемый так же **двоичным разрядом**.
...
Рекомендуется прочитать: [Множество](), [Мощность множества](), [Набор элементов](), [Логарифм]().

Пусть *имеется* некоторый **алфавит `A`** (*множество символов*) **мощности `|A| = n`**, который *используется* при *составлении сообщений* (*упорядоченных наборов символов с повторениями*). 
...
Поскольку *бит* является *минимальной* (*неделимой*) величиной, *количество информации округляется* до *целого числа*. Причём, происходит *округление вверх* (*к большему целому*), чтобы *не было потерь информации*:  
**`i = ⌈ log2(n) ⌉`**, **`I = k • i`**.

Например, в *английским языке* `26` *букв*, то есть `52` *символа* (бывают *строчные* и *прописные буквы*). Тогда если возьмём *английский алфавит* в качестве `A`, то `|A| = n = 52` и *количество информации* в *одном символе алфавита* `A` *равно* `i = ⌈ log2(52) ⌉ = log2(64) = 6` *бит*. В таком случае, некоторое *сообщение длины* `k = 7` *займёт* `I = k • i = 42` *бит*.
