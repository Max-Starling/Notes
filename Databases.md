...
**Естественный ключ** (англ. natural key, business key, domain key) - это *уникальный ключ*, который *полностью зависит* от *свойств* (*природы*) *объекта*, *который* он *идентифицирует*, и *не зависит* от *конкретной базы данных* и *её реализации*.
...
*Примеры* *естественных ключей* всё *те же*, что и *примеры* *потенциальных ключей*: *человека* можно *идентифицировать* по *серии* и *номеру паспорта*, *пользователя* - по *email*, *дом* - по *адресу* и так далее. *Всё* это - *натуральные ключи*, *взятые* из *предметных областей* своих *объектов*.

### Суррогатный ключ

*Разделим понятия реального объекта* (дом, машина, человек, дерево) и его *физического представления* в *базе данных* (набор значений атрибутов).
...
*Примеры суррогатных ключей*: *UUID*, *порядковые номера создания объектов*, *даты создания объектов*.
...
Пример *связи* `M:M`: у *научной статьи* может быть *несколько авторов*, у *автора* может быть *много научных статей*.
