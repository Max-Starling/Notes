...
Реляционная модель данных основывается на понятии *отношения*, которое пришло в немного изменённом виде из теории множеств.

## Кратко о главном из теории множеств
...
**Кортежем** называют *упорядоченный набор объектов* (*не обязательно уникальных*) `(x1, x2, ..., xn)`, где `x1, x2, ..., xn` принадлежат множествам `X1, X2, ... Xn` (не обязательно различным).
...
**Отношение между множествами** `X1, X2, ..., Xn` - это подмножество декартового произведения `X1 × X2 × ... × Xn`, то есть множество, содержащее некоторые элементы (кортежи) множества `X1 × X2 × ... × Xn`.

Например, множество `{ (Сара, 2019), (Рози, 2020) }` является отношением между множествами `N` и `D`.

Отношения могут обладать некоторыми свойствами, наделяя при этом ими свои элементы.
