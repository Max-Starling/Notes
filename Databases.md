...
Пусть имеется `n` атрибутов `a1, a2, ..., an`. Тогда **тело** `B` **отношения** `R` - это *множество кортежей* `K = ((a1, t1, v1), (a2, t2, v2), ..., (an, tn, vn))`.

То есть `B = { K1, K2, ..., Km }`, где `m` - *число кортежей*, которое называется **кардинальным числом**.

Графически, `(a, t, v)` - одна *ячейка таблицы*, а `K` - *строка таблицы*.
