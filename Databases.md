...
*Основной единицей изменения информации* является **бит** (англ. **bi**nary digi**t**), называемый так же **двоичным числом**.

*Бит представляет логическое состояние* с *двумя возможными значениями*. Например, `1` и `0` (используются *чаще всего*), `истина` и `ложь`, `да` и `нет`, `включено` и `выключено` и другие.

Название | Обозначение | Значение
:--: | :--: | :--:
бит | b, bit | 1 bit
байт | B, byte | 8 bit
килобайт | KB | 1000 byte
мегабайт | MB | 1000 KB
гигабайт | GB | 1000 MB
терабайт | TB | 1000 GB
