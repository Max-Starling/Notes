...
Механизм работы с курсором похож на работу с итератором, поскольку курсоры позволяет последовательно обработать каждую строку (каждый документ) из выбранного набора строк (set of rows, документов).

Курсор можно рассматривать как указатель (pointer) на одну строку таблицы (на один документ). Курсор может ссылаться только на одну строку (один документ) в один момент времени, но может перемещаться к другим строкам при необходимости.

Курсоры могут быть явными (объявляются разработчиком) и неявными (не требуют объявлений).
