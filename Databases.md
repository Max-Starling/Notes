...
Примеры: *множество женских имён*: `N = { Ася, Сара, Рози }`, *множество ролей*: `R = { user, admin }`, *множество дат регистрации*: `D = { 2019 2020 }`.
...
**Кортежем** называют *упорядоченный набор объектов* (*не обязательно уникальных*). Вообщн говоря, эти объекты могут принадлежать разным множествам. 
...
*Упорядоченность кортежа* означает, что *кортежи* `(Рози, 2020)` и `(2020, Рози)` *считаются различными*. Соблюдение этого так же важно, как *соблюдение порядка букв в словах* и *цифр в числах*.
