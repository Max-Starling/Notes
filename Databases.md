...
11111111
```
*Утверждение выше* можно легко *доказать комбинаторно*. *Байт* является *упорядоченным набором с повторениями* *длины* `8` с *элементами*, *выбранными* из *множества* `A = { 0, 1 }` (*мощность множества* `A` *равна* `2`), что соответствует *числу размещений с повторениями*: `A(8, 2) = 2^8`.

### Системы измерения информации

Существует *две системы измерения информации* в *битах* (*байтах*).
...
*Префиксы* с *“би”* на конце называют **бинарными префиксами**. Например, `kibi` *означает* `kilo binary`.
