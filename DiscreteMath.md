...
**Мультимножеством** (англ. multiset) называют *неупорядоченный набор объектов* (*не обязательно уникальных*), *обладающих схожими признаками*.

*Обозначение мультимножества совпадает* с *обозначением множества*. Примеры *мультимножеств*: `{ 3, 2, 2 }`, `{ белый, красный, белый }`.

*Число элементов мультимножества* (*включая повторяющиеся* элементы) называют его **мощностью** или **кардинальным числом** (англ. cardinality).
...
**Мультимножеством `A`** называют *множество упорядоченных пар* `(x, M)` вида `{ (x1, M1), (x2, M2), ..., (xn, Mn) }`, где `x` - *элемент мультимножества*, а `N` - *множественность элемента* `x`.
