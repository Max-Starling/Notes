...
- [Мощность (кардинальное число) множества](#мощность-кардинальное-число-множества)
...
**Множество** (англ. set) — *набор* (совокупность) каких-либо *объектов*, наделённых некоторыми *общими признаками* (характеристиками, чертами, свойствами).
...
## Мощность (кардинальное число) множества

**Мощьностью множества `A`** или **кардинальным числом** (англ. cardinality) **множества `A`** называют *число элементов* этого *множества*.
...
*Множества* `A` и `B` называют **равными**, если *все* их *элементы совпадают*. Иначе говоря, множество `A` *содержит все элементы* множества `B` (`B ⊆ A`), а множество `B` *содержит все элементы* множества `A` (`A ⊆ B`). 
...
**Неупорядоченной парой** (англ. unordered pair), иногда просто **парой**, называют *множество*, *состоящее* из *двух элементов*.

*Обозначение неупорядоченной пары элементов* `x` и `y`: `{ x, y }`.

Вспомним, что *порядок следования элементов* в *множестве не важен*, поэтому *пары* `{ x, y }` и `{ y, x }` считаются *одинаковыми*.

### Упорядоченная пара

**Упорядоченной парой** (англ. ordered pair) называют такую *пару двух объектов*, в которой *порядок их следования имеет значение*.

*Обозначение упорядоченной пары* объектов `x` и `y`: `(x, y)`.  

По *определению упорядоченной пары*, для *пары объектов* `x` и `y` *справедливо*: `(x, y) ≠ (y, x)`. 



Определение упорядоченной пары можно ввести при помощи теории множеств:


Пусть *задана неупорядоченная пара* `{ x, y }` элементов `x` и `y`. Множество `{ { x }, { x, y } }` называют **упорядоченной парой**  **элементов `x` и `y`**, то есть такой *парой элементов*, в которой *порядок следования элементов имеет значение*.


Элемент `x` называют **первым элементом** (первой координатой, компонентой) *упорядоченной пары*, элемент `y` – **вторым элементом** (второй координатой, компонентой) *пары*.
