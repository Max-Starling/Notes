...
**Графом** `G` (англ. graph) называют такую *упорядоченную пару множеств* `(V, E)`, в которой *множество* `V` - некоторое *конечное* (счётное) *множество*, а множество `E` представлено *подмножеством декартова произведения* `V × V`, то есть *множеством упорядоченных пар* `(x, y)`, где *элементы* `x, y` (*не обязательно различные*) *принадлежат* множеству `V`.

Множество `V` называют **множеством вершин** (*узлов*) **графа** `G`, а его *элементы* - **вершинами** (*узлами*).
 
Множество `E` называют **множеством рёбер** (*дуг*) **графа** `G`, а его *элементы* - **рёбрами** (*дугами*).
...
В *примерах* и *задачах* для *простоты*  в качестве *вершин графа используют целые числа*, подразумевая при этом, что *вместо чисел* может быть *что угодно*.
