...
1) **Идемпотентность** — такое *свойство* операции, при которотом *повторное применение операции не имеет* никакого *эффекта*. Например, *модуль числа идемпотентен*: `|-1| = ||-1|| = |||-1||| = 1`.  
