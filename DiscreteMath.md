...
*Элемент* `x` **содержится в мультимножестве** `A`, если `m(x, A) > 0`, и **не содержится в множестве `A`**, если `m(x, A) = 0`.

### Саппорт и универсальное множество
...
*Саппорт любого мультимножества* является его *подмножеством* по *определению*.
