...
**Операция** (operation) - это *функция*, которая принимает *любое количество входных значений* (input values), также их называют и имеет *чётко определённое выходное значение* (output value).

Фактически, *определение операции* совпадает с *определением функции нескольких переменных*. *Главным отличием* является *обозначения*: *операции обозначаются специальными символами*, имеют свои *формы записи* и *способы передачи аргументов*.  
...
*В отличие* от *функций* и *отношений*, *каждая операция имеет* своё *уникальное символьное представление* (*иногда* используется *несколько знаков*). Например, *операция сложения обозначается* знаком `+`; *операция умножения* - знаками `•`, `*`, `×`; *логическое отрицание* - знаком `¬` и так далее. 
...
Абсолютное *большинство операций* - *бинарные* и *унарные*.

Напоследок хочется сказать, что в *каждой области знаний* изучаются и используются *свои операторы*. В *программировании* - *арифметические*, *логические* и *строковые операции*, в *математическом анализе* - *операции дифференцирования и интегрирования* и так далее.
...
Отметим, что *унарная операция* из *перечисленных свойств* *может обладать* лишь *свойством идемпотентности*. Например, *модуль числа идемпотентен*: `|-1| = ||-1|| = |||-1||| = 1`.  
...
Логические операции рассматриваются над буле
...
**Конъюнкция** (*логическое “и”*, *логическое умножение*) `x ∧ y` (`x AND y`) - *бинарная операция*, которая *возвращает истинное значение* *тогда и только тогда*, когда её оба *операнда* (`x` и `y`) *истинны*.
