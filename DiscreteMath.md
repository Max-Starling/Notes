...
В случае *объединения двух множеств* `A` и `B` *формула включений-исключений* имеет вид: `|A ∪ B| = |A| + |B| - |A ∩ B|`. Если множества `A` и `B` *пересекаются*, то *каждое* из множеств *содержит* в себе *пересечение* `A ∩ B`, которое *при сложении* `|A| + |B|` *учитывается дважды* и *одно* из *пересечений* необходимо *вычесть*.
