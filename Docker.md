...
Ранее, контейнеры достаточно изолированы от хост-машины, но иногда это можно контролировать.

Если внутри контейнера запущено приложение на порте `3000`, то оно будет доступно только внутри контейнера. Это можно проверить, сделать  запрос на http://localhost:3000 из консоли контейнера. При этом с хост-машины до этого порта добраться нельзя.
...
В данном случае `3000`-ый порт контейнера, который назывляется **выставленным** (exposed), 
