...
## Аргументы ARG
**Инструкция ARG** определяет переменную, которую можно передать во время сборки контейнера.
...
**Инструкция ENV** сохраняет переменную внутри создаваемого контейнера. Таким образом в контейнере переменная доступна как переменная окружения.
