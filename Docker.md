...
При создании контейнера у него по умолчанию нет опубликованных портов. Чтобы опубликовать порт, можно использовать флаг `--publish`, `-p`. Флаг принимает порт хост-машине и выставленный порт контейнера в формате `hostPort:exposedPort`.
```cmd
