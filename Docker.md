...
* Возможность упаковать приложение вместе с его средой запуска. Это позволяет контейнеру запускаться одинаково в разных окружениях (операционных системах) и решает проблему их настройки (подготовки к запуску приложения), а значит на каждом компьютере запуск контейнера происходит одинаково. 
...
Docker Compose использует файлы формата YAML (`.yml`).
...
services:
  bar:
    ...
    ports:
     - "3000:4000"
```
Конфигурация сервиса `baz` эквивалентна конфигурации сервиса `foo`.
```yaml
# docker-compose.yml
version: '3.7'
services:
...
В docker-compose может указываться уже собранный образ (builded image) вместе с командой, которая должна быть запущена в контейнере; или `Dockerfile`, по которому образ будет создаваться.
```yaml
# docker-compose.yml
version: '3.7'
services:
...
**Переменные окружения ENVIRONMENT** передаются в уже запущенные контейнеры. 
```yaml
# docker-compose.yml
client:
  environment:
