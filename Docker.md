...
Ранее, контейнеры достаточно изолированы от окружающего мира, но иногда это можно контролировать.

Если внутри контейнера запущено приложение на порте `3000`, то оно будет доступно только внутри контейнера. Проверить, что оно действительно запущено можно, сделав запрос на http://localhost:3000 из консоли контейнера. При этом с хост-машины получить доступ к приложению не получится.



Чтобы это исправить, нужно опубликовать порт выставленный контейнером порт при помощи флага `-p`.
```cmd
docker run -p 3000:3000 your_image
```

**Инструкция EXPOSE** используется в Dockerfile в целях документации и позволяет понять, какие порты внутри контейнера используются.
```Dockerfile
# Dockerfile
FROM node:12.13.1
...
Также можно выставить порт при помощи флага `--expose`.
```cmd
docker run --expose 3000 your_image
```

Выставление порта не является обязательным, поскольку оно не даёт доступа с хост-машины.

Чтобы выставить порт, необходимо его **опубликовать** (publish), то есть сделать доступным на хост-машине. Такой порт называют **опубликованным** (published).

По умолчанию при создании контейнера у него нет опубликованных портов. Чтобы опубликовать порт, можно использовать флаг `--publish`, `-p`. Флаг принимает порт хост-машине и выставленный порт контейнера в формате `hostPort:exposedPort`.
```cmd
docker run --publish 3000:3000 your_image
```

Имеется также возможность опубликовать все выставленные порты контейнера на случайные порты хост-машины при помощи флага `--publish-all`, `-P`.
```cmd
