...
**Инструкция CMD** описывает команду по умолчанию, которая должна запускаться при запуске контейнера. 
```dockerfile
CMD npm start
```
CMD выполняется не во время сборки, а уже при запуске готового образа.
