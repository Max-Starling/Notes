...
Конкретные инструкции и готовый Dockerfile будут разобраны далее.

### Построение образа

Для **построение образа** (image building) используется команда `docker build`, которая принимает **контекст** (context) — путь к папке, с которой будет происходить работа в Dockerfile.
