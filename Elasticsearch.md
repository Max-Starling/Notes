...
* `_scroll_id` — идентификатор поискового контекста, который используется для получения следующей части результатов.
...
Для получения следующей порции результатов используется запрос следующего вида (в запросе отсутствует название индекса).
```http
...
Когда время жизни контекста поиска истекает, получение дальнейших результатов со `scroll` не работает. Выдаётся следующая ошибка:
```js
 {
