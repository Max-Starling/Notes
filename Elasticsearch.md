...
**Elasticsearch** (эластичный поиск) — распределённый (distributed), RESTful поисковый движок по всему тексту (full-text search).
...
*Индексы* обычно *разделяются* на *несколько подиндексов* (sub-indices), называемых **осколками**, **шардами** (shards). *Каждый шард хранит* какую-то *часть документов индекса*. 
...
*Каждый шард* является *экземпляром движка Lucene*. Таким образом *все данные хранятся в Lucene*, а *Elasticsearch распределённо управляет* этими *данными*.
...
**Репликация** (Replication) — процесс, при котором данные постоянно **реплицируются** (копируются) на один или несколько других серверов. Репликация тоже является стратегией масштабирования баз данных.
...
Поисковый движок Apache Lucene реализует перевёрнутый индекс, используя структуру данных [**список с пропусками**](https://github.com/Max-Starling/Notes/blob/master/Algorithms-Structures.md#список-с-пропусками) (Skip List). Эта структура данных основана на связных списках, но по трудоёмкости сравнима с **B-деревом** (B-tree).
...
Индекс в Lucene очень похож на индекс в конце книги: указывается определённый список ключевых слов (важных определений) и рядом страницы, на которых они упоминаются.
...
Если искать по двум словам одновременно `often` и `alone`, то выдадутся оба документа.
...
* **Числовые** (Numeric): `long`, `integer`, `short`, `byte`, `double`, `float`, `half_float`, `scaled_float`.
...
Elasticsearch предоставляет возможность *хранить одно и то же поле несколькими способами* для *разных целей*. Такое *поле* называется **мультиполем** (multi-field).

Например, *текстовое поле* может быть *одновременно* представлено *типом* `text` для *полнотекстового поиска* и типом `keyword` *для сортировки* и *агрегаций*.

Для определения мультиполей используется свойство `fields`, значением которого выступает объект. Ключом этого объекта является название мультиполя, а в значении описывается тип.
...
Elasticsearch предоставляет возможность группировки нескольких изменяющих данные запросов в один при помощи специального POST-запроса `/_bulk`.
...
```
Наиболее полезная информация лежит в свойстве `hits`: `hits.total.value` - количество всех документов, удовлетворяющих поиску, `hits.hits` - массив самих документов (хранятся в `_source`) и дополнительной информации о них. По умолчанию возвращается только 10 документов (см. [Пагинация](#пагинация)). 
...
Elasticsearch предоставляет **Query DSL** (Domain Specific Language) — предметно-ориентированный язык, позволяющий описывать запрос `query` в формате JSON и отправлять его в теле запроса (request body). Тело можно отправлять даже с GET-запросами.
...
В запрос с `term` также передаётся текстовое, числовое, логическое значение или дата.

В отличие от запроса с `match`, результатом запроса с `term` являются документы, которые содержат **точный терм** (exact term) в указанном поле.
...
Запрос с `range` позволяет задать промежуток значений для конкретного поля. Результатом станут документы, удовлетворяющие промежутку.
...
По умолчанию Elasticsearch ищет по **релевантности**, которая показывает, насколько документ удовлетворяет поисковому запросу. 
...
В **контексте фильтра** (filter context) предложения отвечают на вопрос "Удовлетворяет ли документ запросу?". Ответом является либо да, либо нет, и документ либо включается в выборку, либо нет в соответствии с ответом. 
...
<!-- На языке математики удовлетворение условия здесь означает, что логическая функция (boolean function), определяющая заданные условия, возвращает истинное значение (true). -->
...
* `must` — аналог логического И (AND, пересечение условий).
* `should` — аналог логического ИЛИ (OR, объединение условий).
...
В этом случае `should` просто увеличивает значимость тех документов (увеличивая значение `_score`, которое по умолчанию = 1), которые удовлетворяют заданным условиям, но не исключает из выборки те документы, которые условиям не удовлетворяют.
...
`film 3` является самым старым, но имеет более высокий рейтинг, а рейтинг приоритетнее даты, поскольку указан раньше в параметре `sort`.
`film 2` имеет такой же рейтинг, как и `film 1`, но по дате он новее.

## Обработка больших объёмов данных (scroll)

Один поисковый запрос в Elasticsearch не может обработать более 10000 элементов. 
...
Можно повысить лимит, увеличив значение параметра `index.max_result_window value` в настройках индекса `settings`. Но делать это нежелательно без крайней необходимости, поскольку поисковые запросы занимают память кучи (heap memory) и время пропорционально формуле `max(max_result_window, from + size)`. Если убрать лимит, то лимит памяти будет отсутствовать и кластер может упасть от перегрузки. Лучше получать данные меньшими порциями. 
...
**Контекст поиска** (Search context) — состояние, которое поддерживается в течение всей операции поиска в шарде. Чем больше параллельных (concurrent) поисковых операций выполняется, тем больше объектов поискового контекста существует одновременно. Когда поисковая операция завершается, контекст поиска удаляется.

### Как работать со Scroll API

К обычному поисковому запросу следует добавить query-параметр `scroll` и в качестве значения передать туда время, которое будет жить поисковый контекст до следующего вызова.

Для оптимальной работы поисковый контекст должен жить как можно меньше, но этого времени должно хватать, чтобы обработать результат предыдущего запроса. Поставим 1 минуту в качестве времени жизни поискового контекста.
```http
...
Для запроса со `scroll` контекст поиска создаётся при первоначальном (initial) запросе и живёт для выполнения последующих запросов.
...
Будем анализировать фразу `"- How old are you? - I'm 17."`.
...
Именно *благодаря токенизации доступен полнотекстовый поиск*, ведь *каждый токен индексируется отдельно*.
...
**Фильтр токенов** (Token filter) принимает поток токенов (stream of tokens) и трансформирует его, удаляя, добавляя и изменяя токены.
...
<!-- Можно заметить, что английские стоп-слова перестали игнорироваться, поскольку произошла их перезапись массивом `["old"]`. Чтобы это исправить, можно использовать.
_english_ -->
...
Фильтры, которые могут добавлять многопозиционные токены, называются **фильтрами токенов графа** (graph token filters). Такими являются фильтры `synonym_graph` и `word_delimiter_graph`.

## Нормализация

В то время как благодаря токенизации доступен полнотекстовый поиск, каждый отдельный токен при поиске сравнивается посимвольно.
...
К примеру, токен `Hello` может быть переведён в нижний регистр (`be lowercased`), `users` может быть приведён к его корневому слову `user` (stemmed), `hello` и `hi` являются синонимами и могут индексироваться как одно слово `hello`.
...
**Анализатор поиска** (Search analyzer) анализирует текст поискового запроса (`query`).

В большинстве случаев эти анализаторы должны иметь одинаковый набор правил токенизации и нормализации.

Например, текст `"Hello our USERS!"` может быть преобразован анализатором индекса в `[hello, our, user]`, а текст поискового запроса `"Hi user"` — анализатором поиска в `[hello, user]`.
...
### Разные анализаторы поиска и индекса 

Иногда может появиться необходимость использовать разные анализаторы индекса и поиска.

Например, когда мы хотим убрать из поиска некоторые нежелательные результаты.
...
*Простые токенизаторы* *разбивают текст* на *слова* и задают *тип* `word`. *Другие токенизаторы* могут задавать *типы* `<ALPHANUM>`, `<HANGUL>`, `<NUM>`.
...
**Ориентированные на слова токенизаторы** (Word oriented tokenizer) разбивают текст на отдельные токены, которые являются словами. 
...
* Поскольку общение с Elasticsearch идёт при помощи HTTP-запросов, можно использовать Postman (или что-то похожее) и делать запросы на `http://localhost:9200`.
