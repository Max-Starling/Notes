...
Для поиска начального объекта нужно проверить каждое его поле.
```js
[
...
*Анализатор состоит* из *нескольких составных блоков*: *фильтры символов*, *токенизатор*, *фильтры токенов*.
...
**Токенизатор** (Tokenizer) принимает поток символов (stream of characters) и разделяет их на отдельные токены (individual tokens) и возвращает поток токенов. Часто токенами являются слова. 

То, как токенизаторы встроенных анализаторов разделяют текст на токены (термы), было показано ранее.
