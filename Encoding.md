...
Чтобы не путать с 10 с/с, к числу в 16 с/с обычно добавляют `0x` в начало или `h` (или `₂`) в конец.  
Чтобы не путать с 10 с/с, к числу в 2 с/с обычно добавляют `b` (или `₁₆`) в конец.  
...
110 = 1 * 2² + 1 * 2¹ + 0 * 2⁰ = 6
```
Перевод чисел из 10 с/с в другую с/c.
...
**Маркер последовательности байтов**, **метка порядка байтов** (Byte Order Mark, BOM) — специальный Unicode-символ `U+FEFF`, вставляемый в начало текстового файла или потока, чтобы указать кодировку и порядок байтов, с помощью которых символы Unicode были закодированы.
```css
