- [Системы счисления](#системы-счисления)
  - [Представление чисел в различных с/с](#представление-чисел-в-различных-сс) 
...
**Байт** (англ. `byte`), **октет** (англ. `octet`) — *последовательность из 8 бит*, его можно компактно представить в виде шестнадцатеричного числа.
```css
00000000₂ = 0x0
10110011₂ = 0xB3
11111111₂ = 0xFF
...
**Младший бит** (англ. `least significant`, `low-order`) — крайний справа, **старший бит** (англ. `most significant`, `high-order`) — крайний слева.
...
Например, множество символов `{ A, B, C, D }` используется в английском, немецком и других языках.
...
**Взаимно однозначное соответствие, биекция** (англ. `bijection`) — такое *соответствие между элементами двух множеств*, при котором *каждому элементу первого* множества *соответствует один определённый элемент второго* множества, а *каждому элементу второго* множества — *один определённый элемент первого* множества.
...
В примере выше *кодовыми точками* в кодировках *ASCII* и *Unicode* являются `65, 66, 67, 68` и `U+41, U+42, U+43, U+44` соответственно.
...
* UTF-16: 1-2 кодовые единицы (две требуются для символов, идущих после U+10000)
...
ASCII создавалась для кодирования символов, коды которых вмещались в 7 бит (2⁷ = 128 символов), 8-й бит использовался для ошибок, возникших при передаче данных.  
...
Изначально ASCII была разработана для телекоммуникационного оборудования (печатные машинки для передачи сообщений), поэтому помимо символов, выводимых на печать, содержит управляющие символы, которые не печатаются.
...
```  
2) Определить по кодовой точке количество октетов (байтов, последовательностей из 8 бит; 1 - 4), требуемых для кодирования символа.
```css
0 - 7F          /* 1 октет */
80 - 7FF        /* 2 октета */
800 - FFFF      /* 3 октета */
...
Медиа типы *определены* и *стандартизированы* в спецификации [RFC 6838](https://datatracker.ietf.org/doc/html/rfc6838).
...
**Типом** (англ. `type`) называют *общую категорию данных* (например, `text`, `application`, `video`, `audio`, `image`).
