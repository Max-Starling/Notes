...
**Маркер последовательности байтов**, **метка порядка байтов** — специальный Unicode-символ `U+FEFF`, вставляемый в начало текстового файла или потока, чтобы указать кодировку и порядок байтов, с помощью которых символы Unicode были закодированы.
```css
