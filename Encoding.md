- [Cистемы счисления](#системы-счисления)
...
Например, для текста достаточно часто может быть указана кодировка. Для задания кодировки (англ. `charset`) UTF-8 используется `text/plain;charset=UTF-8`. . При не указании параметра charset, его значение автоматически будет задано, как ASCII (US-ASCII),

Регистр не имеет значения (англ. `case insensitive`) для всего, кроме значения параметра, но, как правило, используют нижний регистр, значение параметра может быть в верхнем регистре.
