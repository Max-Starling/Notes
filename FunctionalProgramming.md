...
curriedSum(1)(3); // 4
```
Каждый *вызов преобразованной функции возвращает новую функцию*, в которой *один параметр фиксируется*. Такая функция называются **частичной**, **частично применённой**.
```js
