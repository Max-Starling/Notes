...
```
Если функция имеет больше одного параметра, её можно обернуть в другую функцию и вызвать в её теле.
```js
divide(pow(3, pow(2, 2)), 3); // 27 = (3 ^ (2 * 2)) / 3
/* преобразуется в */
compose(
...
```
Поскольку функция не может вернуть более одного значения, промежуточные функции композиции могут принять только один аргумент. Симулировать поведение передачи нескольких аргументов можно, если передавать их массивом или объектом.
```js
compose(
  value => divide(value, 3),
  value => pow(3, value),
