...
Очень редко бывают ситуации, когда общего коммита не имеется. По умолчанию Git отказывается сливать такие ветки в одну, но можно использовать флаг `--allow-unrelated-histories`, который при мерже создаст общий коммит для этих веток.

Если общий коммит имеется, то возможны два случая:
* Только в одной ветке были новые коммиты после общего общего коммита. В таком случае одна ветка является продолжением другой и используется стратегия `fast-forward merge`.
