...
Поскольку одна ветка является продолжением другой, указатель последнего коммита короткой ветки сдвигается и становится равным указателю последнего коммита длинной ветки. Таким образом все коммиты из длинной ветки становятся доступными в короткой.
```js
