...
Если обе ветки имеют изменения или используется флаг `--no-ff`, то Git используется стратегию `True merge`. В этом случае создаётся особенный коммит, имеющий двух родителей — **merge commit**. В нём содержатся все изменения, которые были в обеих ветках после общего коммита.

```js
/* до слияния веток, после общего коммита B develop имеет 2 коммита C и D,
