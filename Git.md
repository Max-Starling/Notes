...
```
Если отменить сейчас последний коммит в ветке `master`, то отменится только коммит `D`.
```js
...
При попытке слить в более длинную ветку (`develop`) короткую (`master`) ничего не прозойдёт, Git ответит, что уже используются актуальные изменения.
...
Если обе ветки имеют изменения или используется флаг `--no-ff`, то Git используется стратегию `True merge`. В этом случае создаётся особый тип коммита, имеющий сразу двух родителей, — **merge commit**. В нём содержатся все изменения, которые появились в обеих ветках с момента общего коммита (включая изменения, которые возникли при разрешении конфликтов между двумя ветками). 

Merge commit появляется только в той ветке, в которую вливались изменения.
...
```
Коммит `H` появился только в ветке `master` — в `develop` его нет.

Если из `master` удалить последний коммит, то всё вернётся, как было до слияния веток (в отличии от fast forward стратегии, где все коммиты копируются при слиянии в master и отменяется только последний из них).
...
Если при использовании true merge коммиты сливающихся веток затрагивают одни и те же файлы, возникают конфликты. Git не может автоматически разрешить их, поскольку изменения производились параллельно. Разработчик должен сам решить, какие изменения стоит принять.

Пример конфликта в одном из файлов (в коммите C в файле использована переменная `const`, в коммите E использована `let`).
```js
<<<<<<< HEAD (Current Change)
let name = 'Notes';
=======
...
```
В примере выше Git использует 3 версии
* `===` — состояние до обоих коммитов.
* `<<<` — изменения коммита текущей ветки.
* `>>>` — изменения коммита другой ветки, которая пытается слиться с текущей.
...
Если бы перебазировался не `develop`, а `master`, то изменились бы все коммиты ветки `master` с момента общего коммита с `develop` (в данном случае это только коммит `E`).
```js
/* после перебазирования базовым коммитом для develop стал коммит E,
C* и D* - копии коммитов C и D, но с новым хэшем */
develop (current) |          C —— D
...
По примерам выше понятно, что rebase переписывает историю: все новые коммиты перебазируемой ветки изменяются.

При перебазировании возможны такие же конфликты, как и при `merge`. 
