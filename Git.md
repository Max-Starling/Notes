...
git commit
```
Команда выше открывает редактор по умолчанию, чтобы можно было ввести сообщение коммита. Чтобы не открывать редактор, можно использовать флаг `-m` и передать сообщение напрямую.
```js
...
**HEAD** — указатель на текущую ветку (указатель на последний коммит текущей ветки). `HEAD` может быть только один в один момент времени для всего проекта. 
