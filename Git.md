...
## Состояния файлов
* **Неотслеживаемый** (Untracked). Изменения файла не отслеживаются текущим Git-репозиторием.
...
## Три области Git
* **Рабочая директория** (Working Directory). Файлы распаковываются из сжатой базы данных репозитория и располагаются на локальном диске для чтения и записи.
...
* **Папка .git** (.git directory, Repository). Здесь хранятся метаданные (данные о данных) и Git-объекты текущего проекта.

**HEAD** — указатель на текущую ветку. Текущая ветка является указателем на последний коммит, сделанный в этой ветке. Можно сказать, что *HEAD* — снимок последнего коммита и родитель следующего коммита. 
