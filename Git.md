...
Для управления локальными ветками используется команда `branch`.
```js
/* создание ветки */
git branch staging
...
По сути, ветка является указателем на последний коммит, сделанный в этой ветке. Поскольку последний коммит знает своего родителя, а он своего, по последнему коммиту можно восстановить всю историю коммитов любой ветки.
...
Процесс разработки может происходить параллельно, в разных ветках. Должна быть какая-то основная ветка, которая хранит полную историю приложения. Все остальные ветки создаются от основной и развиваются независимо от неё до тех пор, пока их разработка не завершится. После слияния с основной веткой второстепенные ветки удаляются.
