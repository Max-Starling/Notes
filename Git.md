...
В сравнении со всеми другими системами контроля версий, Git имеет уникальный подход к работе со своими данными. Все эти системы хранят набор файлов и их изменения (дельты) с течением времени в виде списка.
...
Где `delta` — список изменений файла, прочерк — отсутствие данных об изменениях.

Git вместо того, чтобы рассматривать различия, рассматривает данные в качестве **потока снимков** (stream of snapshots). Каждый коммит означает сохранение определённого состояния проекта, система запоминает, как выглядит каждый файл (делает снимок) и сохраняет ссылку на этот снимок. Если файл не изменился, то Git не запоминает его вновь, а создаёт ссылку на идентичную версию файла из предшествующего снимка. 
...
Выделенные курсивом версии файлов не менялись, поэтому были использованы ссылки на версии из предыдущих снимков.

Благодаря описанному выше подходу Git похож на файловую систему и это даёт преимущества при работе .
...
Похожими на коммит понятиями являются **версия** (version) и **снимок** (snapshot).
...
Рассмотрим случай, когда *целевая ветка впереди* (длиннее) *текущей*. Тогда *при слиянии веток указатель на последний коммит текущей ветки сдвигается на последний коммит целевой ветки*. После такого слияния *истории двух веток становятся идентичными*.
```js
...
Если обе ветки при слиянии имеют свои коммиты с момента общего коммита или если стоит флаг `--no-ff`, то используется **true merge** (**3-way merge**). В этом случае создаётся особый тип коммита, имеющий сразу двух родителей, — **merge commit**. В нём содержатся все изменения, которые появились в обеих ветках с момента общего коммита (включая изменения, которые возникли при разрешении конфликтов между двумя ветками). 
...
Когда мы указываем ветку в качестве `<base>`, берётся верхушка (последний коммит) этой ветки.
...
По примерам выше понятно, что rebase переписывает историю: все новые коммиты перебазируемой ветки изменяются. Отменить последствия `rebase` невозможно (хеш старых коммитов утерян).
