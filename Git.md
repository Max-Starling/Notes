...
**Перебазирование** (Rebasing) — процесс перемещения последовательности коммитов к новому базовому коммиту.
```js
git rebase <base>
```
...
master            |   A —— B —— E
```
В примере выше видно, кто перебазирование перезаписывает историю. Все коммиты ветки `develop` были изменены, из чего следует, что не стоит использовать `rebase` в общеиспользуемых ветках.
