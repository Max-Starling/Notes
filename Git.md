...
*После создания тэга* его необходимо *передать его* на *удалённый резпозиторий* (*автоматически он не передаётся так же*, как и *ветка*).
```yml
...
v1.0.1
```
4) Создаём новый тэг в ветке `production`.
```yml
$ git tag -a v1.1.0 -m "My version 1.1.0 (Feb, 11 2022)"
```
