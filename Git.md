...
Если из `master` удалить последний коммит, по всё вернётся, как было до слияния веток (в отличии от fast forward стратегии, где все коммиты копируются при слиянии в master и отменяется только последний из них).
