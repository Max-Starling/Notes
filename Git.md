...
Когда *продукт готов к использованию*, происходит его *выпуск* - **релиз** (англ. `release`), то есть *продукт становится доступен пользователям* (например, *в виде приложения* в App Store или *вебсайта*). Со временем *продукт улушается*, а значит происходят *новые релизы* - и *пользователи* получают *доступ к новым функциям приложения*. *Каждая конечная (готовая) версия продукта* называется его **версией** (англ. `version`).

*Git* сам по себе является *системой контроля версий*: он *сохраняет, фиксирует состояние приложения* (*делает контрольные точки приложения*) с *каждым коммитом* и *отслеживает разницу между двумя состояниями приложения*, то есть *дельту изменений между двумя коммитами*.

Каждый коммит в Git в некотором понимании является новой версией приложения, но это версия обычно ещё не готова к релизу, а значит её не увидят пользователи и её нельзя называть новой версией приложения как таковой.

По этой причине, в Git помимо коммита и его хэша (уникального идентификатора коммита) существует альтернативный способ указать версию приложения - **тэг** (англ. `tag`) и *набор команд для работы с тэгами*.

### Семантическое версионирование, формат версий

Обычно версии нумеруют в формате `<X>.<Y>.<Z>`, где `<X>`, `<Y>`, `<Z>` - *целые неотрицательные числа*.
...
При создании тэга можно задать коммит, к которому будет прикреплён тэг
```yml
...
*После создания тэга* необходимо *передать его* на *удалённый резпозиторий* (*автоматически он не передаётся так же*, как и *ветка*).
```yml
...
Для *отображения информации* о конкретной *версии* используется команда `git show`:
```yml
$ git show <tagname>
```
```yml
