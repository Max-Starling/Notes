...
Большинство тегов — *парные*. Один — **открывающий** `<tagName>`, другой — **закрывающий** `</tagName>`. Между тегами располагается контент, который необходимо отобразить.
```html
<div></div>
<p>Hello</p>
```
...
**Парные теги** имеют открывающий и закрывающий тег, перед именем закрывающего ставится слэш `/`:  
```html
<div></div>
<p>text</p>
<span>inline</span>
...
Соответствуют блочным и строчным элементам.  
...
```
Строчные элементы обычно хранят только текст (текст, изображение), не могут содержать блочные элементы. (`<span>`, `<img>`)
```html
<span>text</span>
<img src="/*...*/">
```
...
С помощью этих тегов браузерам проще индексировать страницу, отделить важный контент от дополнительной информации.  
...
**Устаревшие тэги** (obsolete) были ранее запрещены и удалены или могут быть удалены из браузеров в любой момент (хотя некоторые браузеры до сих пор могут их поддерживать).
```html
