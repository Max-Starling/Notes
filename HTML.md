...
HTML сам по себе не является полноценным языком программирования, поскольку не имеет возможностей типичных языков программирования: в нём нет условных операторов, нельзя создавать и использовать переменные. 

По сути, HTML-разметка является обычным тексом, строкой.

Изменения HTML чаще всего требуются на клиенте, и это задача ложится JavaScript. JavaScript работает с DOM, древовидным представлением HTML. Каждому HTML-элементу ставится в соответствие DOM-элемент. В DOM имеется множество методов, позволяющих искать, добавлять и удалять DOM-элементы, получать и изменять их атрибуты, стили и многое другое.

На сервере же нет необходимости в методах, которые предлагает DOM API. Здесь чаще всего стоит задача сгенерировать HTML (строку) и передать её клиенту, где он сможет уже манипулировать ей при помощи JavaScript. 

При генерации HTML на сервере чаще всего необходимо передать в разметку некоторые переменные. 

В самом простом случае можно воспользоваться шаблонными строками.
```jsx
const generateHelloHtml = (name) => `
  <html>
    <head>
      <title>Hello</title>
    </head>
