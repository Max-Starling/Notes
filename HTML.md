...
Существует три режима браузеров: **режим совместимости** или **режим "причуд"** (Quirks mode), **режим стандартов** (Standards mode) и **почти режим стандартов** (Almost standards mode).

В случае отсутствия Doctype или невалидного Docktype все браузеры переходят в *режим совместимости* (Quirks Mode). В этом случае каждый браузер заменяет современную функциональность своими заглушками и происходят различные причуды (quirks), варьирующиеся от "ничего не происходит" до "всё падает с ошибками".

Режим совместимости предназначен для отображения веб-страницы подобно старым браузерам. В режиме совместимости игнорируются стандарты HTML и CSS, и поведение браузеров становится непредсказуемым, поскольку каждый браузер по-разному реализует функциональность режима совместимости. Таким образом устаревшие сайты могут ещё достаточно долго поддерживаться, не переписывая кодовую базу под новые стандарты.
...
Наличие валидного Doctype выступает гарантией того, что современные браузеры обработает документ одинаково. 

Для проверки текущего режима в браузерах имеется свойство `document.compatMode`, которое выдаёт `CSS1Compat`, если установлен режим стандартов, и `BackCompat`, если установлен режим совместимости.
