...
В случае отсутствия Doctype или невалидного Docktype все браузеры переходят в **режим совместимости** (Quirks Mode).

Режим совместимости предназначен для отображения веб-страницы подобно старым браузерам. В режиме совместимости игнорируются стандарты HTML и CSS, и поведение браузеров становится непредсказуемым, поскольку каждый браузер по-разному реализует функциональность режима совместимости.

Пример невалидного Doctype, переводящего браузер в режим совместимости.
```html
<!DOCTYPE html PUBLIC>
```
Пример валидного Doctype, переводящего браузер в режим совместимости.
```html
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 3.2 Final//EN">
```

Наличие валидного Doctype выступает гарантией того, что разные браузеры обработают документ одинаково.

Для проверки текущего режима есть свойство `document.compatMode`, которое выдаёт `CSS1Compat`, если установлен `Standard`, и `BackCompat`, если установлен `Quirk`.
