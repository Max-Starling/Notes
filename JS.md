...
*Метод [[Call]] выполняет код*, связанный с *его* функциональным объектом.  

*Вызывается* через *выражение вызова функции*. 
```js
object()
```

*Аргументы* метода *[[Call]]*: значение *this* и *список аргументов*, переданных *функции выражением вызова*.  

Объекты, которые *реализуют* этот внутренний метод *[[Call]]*, называются **вызываемыми** (collable).

### Метод [[Construct]]

*Метод [[Construct]] cоздаёт объекты*.

*Вызывается* при помощи *операторов new* и *super*.

*Первый аргумент*: *список аргументов оператора*.  

*Второй аргумент*: *объект*, к которому изначально был *применён оператор new*.
