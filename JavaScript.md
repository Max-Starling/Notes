...
### Object.observe()
Для отслеживания мутаций раньше был доступен метод `Object.observe()`. Сейчас он запрещен (deprecated), поскольку были добавлены другие, более общие способы отслеживать мутации.
```js
Object.observe(obj, callback);
```
* `obj` —  объект под наблюдением.
* `callback` — функция обратного вызова, принимающая массив объектов с изменениями.

Метод `Object.observe` работает асинхронно. Он возвращет массив всех изменений в том порядке, в котором они происходили при выполнении скрипта.
```js
const foo = {
  a: 17,
  b: 'notes',
