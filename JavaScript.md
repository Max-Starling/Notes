- [Основы JavaScript](#основы-javascript)
  ...
  - [Замыкание и область видимости](#область-видимости-лексическое-окружение-замыкание)
...
console.log(foo); // 7
```
*Переменная var*, находящаяся *вне* каких-либо *функций*, размещается в *глобальном объекте*. Например, это может быть `window`.
```js
var foo = 'notes';
...
Если *преобразование типов* происходит *автоматически*, то оно называется **неявным**. Этот тип преобразований *характерен для JavaScript*. Обычно *такие преобразования* происходят при *выполнении операций* между *операндами разных типов*. 
```js
console.log(1 + [] + {} + 'notes'); // "1[object Object]notes"
console.log(1 - []); // 1
```
...
По умолчанию параметр `preferredType` имеет значение `'number'`. Если передать `'string'`, то шаги алгоритма с `valueOf()` и `toString()` *меняются местами*.
...
Что такое переменная? Переменная - это именованная область памяти для хранения данных (информации). 
...
Стоит избегать явного использования глобальной области видимости, если это возможно, и стараться использовать только локальную область видимости. В идеале следует писать код так, чтобы внешняя область видимости не содержала тех переменных, которые характерны для какой-то определённой внутренней области видимости. С одной стороны, это отвечает принципу инкапсуляции: скрываются детали реализации (выставляется наружу только то, что необходимо). С другой стороны, это отвечает принципу модульности: в коде появляются самодостаточные блоки (модули, фичи, пакеты, компоненты - их называют по-разному), которые хранят всё нужное внутри себя, не имеют внешних зависимостей и, таким образом, могут быть довольно легко экспортированы в другое место.
...
**Замыкание** (англ. `closure`) — это функция вместе со ссылками на её *окружение*, называемое **лексическим окружением** (Lexical Environment). По сути говоря, любая функция в JavaScript представляет собой замыкание.
...
**Всплытие** (англ. `hoisting`) — *поведение* JavaScript, *размещающее объявления* (англ. `declarations`) *вверху текущей области видимости* (англ. `current scope`).
...
> Джек Лондон — это американский писатель, который обрёл известность благодаря своим приключенческим рассказам и романам. Те, кому довелось знать его лично, описывали его как мужественного, отважного, целенаправленного и решительного человека...
Как видно, в первом предложении указано имя человека. И во втором предложении нам уже ясно, что местоимение "его" ссылается на Джека Лондона.
...
Если убрать первое сообщение из примера выше, то тогда непонятно, о ком идёт речь. В таком случае говорят о нарушении целостности контекста или вырывании из контекста.
> Те, кому довелось знать его лично, описывали его как мужественного, отважного, целенаправленного и решительного человека...
...
В JavaScript *массивы* и *функции* также являются *объектами*, но оператор `typeof` имеет *тип* `"function"` для *удобства*.  
 ```javascript
...
```
Оператор `typeof` считает `null` *объектом*, что является *врождённой ошибкой* JavaScript, которую *не исправляют* в целях *поддержки совместимости* с предыдущими версиями.  
...
**Оператор void** — *унарный* оператор, *выполняющий принимаемое выражение* и *возвращающий undefined*.  
...
Оператор `delete` *не связан* с *очисткой памяти*. *Очистка памяти* осуществляется *сборщиком мусора* при *разрыве ссылок*.
...
**Клонирование через Object.create()** не имеет смысла, поскольку `Object.create(proto)` создаёт новый объект, используя существующий объект `proto` в качестве прототипа для нового.
```js
const obj = { a: 7 };
...
Во-вторых, требуется поддержка функции `uneval`, имеющаяся только у Firefox, но даже в нём это может не сработать из-за политики безопасности контента (Content Security Policy): `EvalError: call to eval() blocked by CSP`.
...
**Глубокое клонирование** (deep clone) подразумевает копирование свойств на всех уровнях, то есть на каждом уровне вложенности вместо передачи по ссылке создаётся новый объект с теми же свойствами.
...
* `{ a: {}}` и `{ a: {}}` считаются неэквивалентными, поскольку их неглубокие свойства `a` представлены объектами с разными ссылками (`{} !== {}`).
...
```
Неглубокое сравнение применяется в Redux: `shallowEqual(oldState, newState)`, чтобы выяснить, изменился ли *State*. 
Именно поэтому очень важно *не мутировать State*: изменяются и новый, и старый State одновременно — неглубокое сравнение не видит различий между ними.
...
Одним из способов провести глубокое сравнение является JSON-сериализация (сравниваются получившиеся строки). Это достаточно быстрый способ.
```js
...
### Object.observe()
Для отслеживания мутаций раньше был доступен метод `Object.observe()`. Сейчас он запрещён (deprecated), поскольку были добавлены другие, более эффективные способы отслеживать мутации.
```js
...
Метод `Object.observe()` работает *асинхронно*. Он возвращает массив объектов со всеми изменениями. Объекты в массиве расположены в том же порядке, в котором происходили изменения при выполнении скрипта.
```js
const foo = {
  a: 17,
  b: 'notes',
...
При помощи `Proxy` можно временно откладывать совершение действия, производить валидацию, отмену действия, устанавливать значение по умолчанию (если устанавливаемое значение невалидно), логирование и многое другое.
```js
...
* `Reflect.get(target, property)` эквивалентно `target[property]`.
* `Reflect.set(target, property, value)` эквивалентно `target[property] = value`.
и так далее.
...
const proxy = new Proxy(userTable, {
  get(target, property) {
    if (property in target) {
      return Reflect.get(target, property); // эквивалентно target[property];
    } else {
      return guest;
    }
  },
...
В массиве по некоторым индексам могут лежать пустые элементы (`empty`).
```js
const foo = [, 0, 1, 2]; 
...
*Массив* в JavaScript имеет методы, характерные для **двухсторонней очереди** (deque, double ended queue), что позволяет достаточно просто добавлять и удалять элементы на обоих концах массива.
```js
let foo = [2];
...
Поскольку в *JavaScript примитивные значения копируются напрямую*, а *объекты передаются по ссылке*, имеем следующее *поведение параметров функции*.
```js
const foo = 1;
const bar = { a: 1 };
const baz = { c: 1 };
...
С `async` такой ошибки не возникает, поскольку `async` вообще *не является оператором* и не рассматривается отдельно от `function`.
```js
...
Ошибка ниже связана с проведением браузерами *политики безопасности контента* (Content Security Policy), которая помогает *избежать* многих *потенциальных XSS* (cross-site scripting) *атак*.  
...
**Функция-конструктор** (constructor function), или просто **конструктор** (constructor), — *функциональный объект*, поддерживающий *внутренний метод [[Construct]]*.
...
**Метод [[Construct]] (argumentsList, newTarget)** *создаёт и возвращает объекты*.
...
Слово *инстанцирование* применяется *к классу*, *создание* (creation) - *к объекту*.  

Несмотря на то, что *функции* в JavaScript являются *объектами*, в то же время они могут быть и *классами*, поэтому к ним и *применяется* слово *инстанцирование*.

*Функциональные объекты инстанцируются* при помощи:
```js
...
```
#### I этап интерпретации - инстанцирование
...
4) *Создать конструктор* с помощью `MakeConstructor(closure)`.  
5) *Вернуть closure*.
