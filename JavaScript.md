...
**Неглубокое клонирование** (shallow clone) подразумевает копирование прямых свойств оригинального объекта в новый объект. Если свойство само является объектом (`prop: {}`), то оно передаётся по ссылке (оригинальное и скопированное свойство ссылаются на один объект). 

*Прямое свойство объекта* `obj`: `obj.prop`, *непрямое свойство*: `obj.prop.nestedProp`. Непрямые свойства объекта клонируются автоматически, поскольку содержатся в объектах, являющихся прямыми свойствами.
