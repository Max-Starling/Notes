...
Прежде, чем приступать к понятию контекста в рамках JavaScript, разберёмся, что же такое контекст в широком смысле этого слова.

**Контекстом** (англ. `context`) называют совокупность фактов и обстоятельств, в окружении которых происходит какое-либо событие, существует какое-либо явление или какой-либо объект. Чем больше мы знаем о чём-то, тем точнее мы можем воспроизвести это. Чем больше рассказчик даёт вам деталей, тем детальнее вы представляете то, что он видел своими глазами..
...
Если вернуться к языку JavaScript, то под "контекстом" обычно подразумевают **контекст выполнения** (англ. `Execution Context`, `EC`). 
...
Теперь рассмотрим, что такое контекст в JavaScript.
