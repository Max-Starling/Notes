- [Основы JavaScript](#основы-javascript)
  ...
  - [Функции под капотом JavaScript (из спецификации)](#функции-под-капотом-javascript-из-спецификации)
...
*Регистр* имеет значение (`ALL` и `all` - разные переменные).  
...
Результатом дейсвтия оператора являетя строка.  
 ```javascript
