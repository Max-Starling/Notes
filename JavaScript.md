...
При помощи `Proxy` можно: откладывать совершение действия, валидировать его, отменять, устанавливать по умолчанию в случае чего и многое другое.
```js
...
Result: "6" */
```
На примере выше стоит отметить, что прямое взаимодействие с проксируемым объектом не имеет никакого эффекта — нужно всегда использовать созданный `Proxy` вместо него.

### Reflect

**Reflect** — встроенный JavaScript-объект, предоставляющий методы для всех тех действий, которые может перехватывать `Proxy` (для каждой ловушки).

*Reflect* не является функциональным объектом, поэтому его нельзя вызвать как функцию или использовать как конструктор. Все его методы статические.
...
**Псевдомассив** (pseudo-array) — обычный *объект*, который аналогично массиву в качестве свойств имеет индексы `0, 1, 2, ...` и свойство `length`, но при этом не имеет доступа к стандартным методам `Array` (`find`, `includes`, `forEach`, `slice`, ...).
