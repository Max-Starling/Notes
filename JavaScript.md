...
**Неглубокое клонирование** (shallow clone) подразумевает копирование прямых свойств оригинального объекта (`obj.prop` — прямое, `obj.prop.nestedProp` — нет) в новый объект. Если свойство само является объектом (`{ prop: { /* ... */ }}`), то оно передаётся по ссылке (оригинальное и скопированное свойство ссылаются на один объект). 
