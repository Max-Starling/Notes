...
console.log(i); // 10
```
*Переменную var* *можно использовать до* её *инициализации значением*. Такое поведение называется *всплытием*. Всплывает только определение переменной, а её значением временно становится `undefined`.
```js
