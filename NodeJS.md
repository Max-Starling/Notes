...
Чаще всего блокирующими операциями являются **операции ввода/вывода** (Input/Output, I/O): работа с файлами (пример выше), с базами данных и так далее. JavaScript, являясь высокоуровневым языком, не работает с файлами напрямую, перекладывая это на библиотеку `libuv` и ей подобные.
