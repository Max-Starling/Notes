...
С учётом того, что JavaScript ориентирован на события, одни из которых могут быть намного важнее других, очень важно не терять время на ожидание завершения запущенной операции, потому что во время ожидания могут быть выполнены другие, более важные вещи. По этой причине в JavaScript (а следовательно и в NodeJS) используется асинхронный подход.

Но если просто сделать функции работы с файлами в примере выше асинхронными, станет неизвестно, какая операция выполнится раньше (удаление файла или чтение, вычисление результата или его вывод). Нарушается последовательность блока кода, который должен был строго ей следовать.
```js
