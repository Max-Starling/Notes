...
В NodeJS используется *кроссплатформенная библиотека* `libuv`, нацеленная на *асинхронный ввод-вывод*. `libuv` по умолчанию имеет 4 потока, вместе называемых `libuv thread pool`. Он выполняет те операции, которые считаются слишком трудоёмкими для NodeJS.
