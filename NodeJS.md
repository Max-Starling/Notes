...
**Ввод** (Input) — данные или сигналы, получаемые системой. 

Ввод можно рассматривать как команду, которую получает система. Например, создание текстового файла `NodeJS.md` с текстом `Notes`.
```js
const fs = require('fs');
...
Вывод можно рассматривать как результат действия системы. Например, текст прочитанного системой файла.
```js
const fs = require('fs');
