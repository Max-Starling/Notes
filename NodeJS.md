...
`libuv` предоставляет возможности для работы с сетью (network I/O), файлами (File I/O), операционной системой (Operating System) и другими вещами. 

Event Loop является чатсью Libuv и он однопоточен.

Reactor pattern

Работа с сетью: TCP и UDP обработчики. Операционная система (Thread pool не используется).
Работа с файлами: stream и pipe (потоки ввода-вывода). Thread pool (работает только с file I/O).
