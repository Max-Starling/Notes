...
Во время выполнения основного скрипта вызываются асинхронные операции (таймеры, операции ввода-вывода), происходит подписка на события. Все эти операции имеют колбэки, которые помещаются в специальные очереди по их завершению.

В цикле событий имеется несколько фаз, каждая из которых имеет очередь колбэков определённого типа.
...
Между каждой фазой есть две дополнительные фазы:
