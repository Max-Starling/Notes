...
**Синхронный подход** подразумевает синхронное (последовательное) выполнение операций в порядке их указания в коде. Следующая операция не может начать выполняться, пока не завершится текущая.

Большинство операций выполняется синхронно (логирование, арифметические операции и многое другое).
```js
console.log('start');
const sum = (a, b) => a + b;
console.log(sum(1, 7));
...
**Асинхронный подход** подразумевает последовательный запуск операций в порядке их указания в коде, при котором следующая операция не дожидается завершения текущей перед своим запуском (выполнение текущей операции не препятствует запуску следующей).
...
Нет необходимости выполнять все операции асинхронно, желательно асинхронно выполнять те операции, которые затрагивают функциональность не JavaScript-кода и таким образом могут занять неопределённое время. 
...
Но в *контексте NodeJS* *понятие операций ввода-вывода относятся* к *работе с диском* (файлами), *сетью* (network, HTTP-запросы), *базой данных*, *DNS*. Эти *операции* достаточно *трудоёмкие* и *выполняются за пределами JavaScript* такими *библиотеками*, как `libuv`.
...
Операции, которые приводят к *блокировке основного потока* называют **блокирующими операциями ввода-выввода** (Blocking I/O).
...
Операции ввода-вывода, которые не блокируют основной поток, называют **неблокирующими операциями ввода-вывода** (Non-blocking I/O). К ним обычно относят *асинхронные операции*.

JavaScript-код выполняется довольно быстро и считается неблокирующим. Тем не менее, такая блокировка возможно, если запущен код, который выполняется бесконечно (бесконечный цикл, бесконечная рекурсия). 
...
Тем не менее, просто сделать все функции асинхронными не получится, некоторые операции всё же должны выполняться последовательно. Если заменить синхнронность на асинхнронность в примере выше, то удаление файла может начаться раньше, чем закончится чтение, а вывод результата — раньше его получения. 
```js
...
**Стек вызовов** (Call Stack) — вспомогательная структура в виде LIFO-очереди (Last In First Out), позволяющая следить за выполнением JavaScript-кода. А точнее узнать, какая функция выполняется в данный момент и в пределах какой функции она вызвана.
...
Стек вызовов так же, как и цикл событий, является внутренней реализацией и не доступен для разработчика.
...
foo();
```
Когда начинает выполняться скрипт, он помещается в стек вызовов.
```js
/* Call Stack */
- main()
```
...
**Цикл событий** (Event Loop) — механизм, позволяющий *NodeJS выполнять неблокирующие операции ввода-вывода*. Это означает, что как только NodeJS встречает операцию, которая может заблокировать основной поток, он выгружает её в ядро системы, если это возможно, и продолжает выполнение, не ожидая завершения операции. В отличие от NodeJS большинство ядер являются многопоточными, что ускоряет обработку операций  ввода-вывода.

Цикл событий запускается только после завершения работы основного скрипта, поэтому он не блокирует основной поток при запуске сервера. 

Каждый колбек цикла событий исполняется в основном потоке при помощи стека вызовов. При этом цикл событий не может ничего делать, пока стек вызовов не пуст.
...
*Цикл событий* прежде всего является *циклом*. Один полный обход цикла событий* называется **тиком** (tick).
...
* **Опрос** (Poll) — колбэки завершённых операции ввода-вывода (I/O). На этой стадии выполняется большинство колбэков.
...
Между каждой фазой есть две промежуточной фазы:
...
* Программист запускает приложение, точкой входа в которое является скрипт `app.js`.
```node
node app.js
```
