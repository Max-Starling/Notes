...
Но в контексте NodeJS понятие операций ввода-вывода относятся к взаимодействиям с диском, сетью (network), базами данных, DNS и так далее. Эти операции достаточно трудоёмкие и выполняются за пределами JavaScript (такими библиотеками, как `libuv`).
