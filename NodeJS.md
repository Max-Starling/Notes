...
```
Стоит отметить, что класс `EventEmitter` работает синхронно. Поэтому важно, чтобы подписка на событие `on()` находилась в коде раньше его генерации `emit()`, иначе событие не обработается.
...
Сам подход появился в функциональном программировании ещё задолго до появления NodeJS и назывался **Continuation-passing Style** (CPS) и подразумевал, что *управление* (какой-то набор инструкций) *передавалось* далее *в форме продолжения* (continuation). То есть одна функция могла принять другую функцию — **функцию продолжения** (continuation function). По завершении своей работы вместо возвращения стандартного результата она вызывала функцию продолжения, передавая ей результат в качестве аргумента.

Большинство методов NodeJS сейчас использует колбэки. Когда асинхронная, блокирующая операция поступает на вход и запускается, колбэк сохраняется в памяти и его выполнение откладывается, последующий код может продолжить своё выполнение. Как только основной код и асинхронная функция завершают своё выполнение, колбэк достаётся из памяти и выполняется в том же потоке, в котором выполнялся основной код. Такой подход весьма эффективен, поскольку значительно увеличивает пропускную способность и скорость NodeJS-приложений.
...
Большинство асинхронных методов в *NodeJS* следуют *идиоматическому* (свойственному языку) *паттерну* **Error-first callback** (сперва ошибка), позволяющему довольно просто узнать, произошла ли ошибка при выполнении операции. 
...
* Если *операция завершается успешно*, *первым аргументом* станет `null`, а *результат операции* (данные) будет передан *последующими аргументами* (чаще всего только вторым аргументом).
```js
...
Как мы уже выяснили, использование колбэков помогает сделать код асинхронным.

<!-- асинхронные операции вызываются, но дальнейший код не ждёт их завершения, продолжая свою работу. В итоге ошибки и данные из функций обратного вызова не могут быть возвращены: их необходимо обрабатывать на месте. -->
...
До их появления разработчики пытались уменьшить негативное влияние *Callback Hell*, вынося функции обратного вызова в переменные или даже в отдельные модули.

Если вынести функцию не удаётся (например, в ней используются переменные из замыкания), можно использовать именованные функции вместо анонимных. Это особенно может упростить понимание кода, когда асинхронная функция принимает несколько функций обратного вызова.
```js
...
Есть *другой способ разрешения Callback Hell*: *промисификация асинхронных функций* и объединение их в цепочку промисов (promise chaining). 
...
Например, промисифицируем асинхронные функции создания, чтения и удаления файла.
```js
const fs = require('fs');
...
/* промисификация с помощью promisify */
...
Начиная с Node v11.0.0 можно использовать встроенные промисы.
```js
...
Эта конструкция позволяет писать асинхронный код в синхронном стиле, поскольку под капотом оборачивает его в промисы.

`await` дожидается выполнения промиса или цепочки промисов (вызывает `.then` до тех пор, пока не вернётся ошибка или примитивное значение). Использование `await` доступно только внутри `async`.

`async` оборачивает результат выполнения функции или метода в промис. 
...
asyncFn(); // Promise
```
При использовании `async..await` ошибка или данные так же, как и в случае с функциями обратного вызова, не могут быть возвращены в код, в котором была вызвана функция: в данном случае они не покидают промис.
```js
const asyncFn = async () => {
  try {
    const data = await readFile('input.txt');
...
**Стек вызовов** (Call Stack, Stack) — вспомогательная структура данных в виде LIFO-очереди (Last In First Out), хранящая информацию о том, в каком месте в коде выполняется скрипт в данный момент. С помощью стека вызовов можно узнать, какая функция выполняется в данный момент и в пределах какой функции она вызвана.
...
- main()
```
Каждая вызванная в скрипте функция добавляется в стек.
```js
/* Call Stack */
- foo()
- main()
```
Если внутри неё вызывается другая функция, то она помещается в стек поверх предыдущей.
```js
/* Call Stack */
- bar()
- foo()
...
Цикл событий не может работать, пока стек вызовов не пуст. Он ждёт, пока освободится стек вызовов и, если в очереди есть готовые к исполнению колбэки, берёт первый из них и помещает его в стек. Когда стек снова освобождается, в него помещается следующий колбэк и так далее.

Как в браузере, так и в NodeJS цикл событий скрыт от программиста, является частью внутренней реализации.
...
Во время выполнения основного скрипта вызываются асинхронные операции (таймеры, операции ввода-вывода), происходит подписка на события. Каждая такая операция имеет колбэк, который по завершении операции в зависимости от её типа помещается в характерную очередь. Каждая очередь представляет собой определённую **фазу** (phase) цикла событий.
...
* **Опрос** (Poll) — *ожидание новых колбэков* (таймеров, ожидающих колбэков) и их выполнение. Здесь выполняется большинство *колбэков завершённых операций ввода-вывода* (I/O).
...
Когда цикл событий останавливается на определённой фазе, он выполняет все её колбэки до тех пор, пока они не закончатся или не будет достигнут лимит их выполнения. Затем цикл событий переходит к следующей фазе.
...
```
Первым параметром всегда передаётся полный путь к NodeJS, вторым - полный путь к исполняемому js-файлу. 
```cmd
node index.js test
```
```js
