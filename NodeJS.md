...
**NodeJS** — серверная среда выполнения JavaScript, построенная на JavaScript-движке V8, который используется в Google Chrome.
...
Некоторые операции могут занимать довольно долгое время, что приводит к блокировке основного потока, поэтому такие операции называют **блокирующими операциями ввода-выввода** (Blocking I/O). Операции, выполняющиеся достаточно быстро (арифметические операции, `console.log()` и многие другие), называют **неблокирующими операциями ввода-вывода** (Non-blocking I/O), поскольку они не блокируют основной поток.
