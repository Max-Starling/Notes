...
Но просто так сделать все функции асинхронными (не дожидаться их выполнения) не получится. 

Если в примере с синхронной работой с файлами выше заменить все функции на асинхронные, может нарушиться последовательность части кода, которая должна строго ей следовать. Удаление файла может начаться раньше, чем закончится чтение, а вывод результата произойдёт раньше его вычисления. 
```js
