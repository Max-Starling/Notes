...
В контексте NodeJS опяните операций ввода-вывода в основном относятся к взаимодействиям с системным диском и сетью (network), которые поддерживаются библиотекой `libuv`.
...
```
Такое ветвление называют **callback hell**. Чем больше последовательных асинхронных операций, тем сложнее работать с таким кодом.
