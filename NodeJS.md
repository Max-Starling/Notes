...
Работа с ядром системы происходит при помощи библиотеки `libuv`. `libuv` имеет фиксированное количество потоков (по умолчанию 4), вместе называемых `thread pool`. NodeJS отправляет `libuv` очередь задач, откуда потоки из `thread pool` берут (pull) задачи и исполняют их. Когда какой-то поток завершает задачу, он информирует об этом NodeJS и колбэк выполненной задачи поступает в очередь на исполнение.
...
Цикл событий не может работать, пока стек вызовов не пуст. Если стек вызовов пуст и есть колбэки, которые ждут выполнения, цикл событий помещает один из них в стек вызовов. Когда колбэк выполняется, цикл событий помещает в стек следующих колбэк и так далее.
