...
**Блокировка** (Blocking) — ситуация, при которой блок JavaScript-кода не может быть запущен, поскольку основной поток находится в ожидании результата выполнения не JavaScript-кода.
...
JavaScript в браузере и NodeJS на сервере используют асинхронный подход, поскольку они ориентированны на события. На клиенте важно быстро реагировать на действия пользователя, на сервере — отвечать на запросы клиента. Нельзя терять время на ожидание завершения операций, которые выполняются вне основного скрипта.

Но просто так сделать все функции асинхронными (не дожидаться их выполнения) не получится. В примере выше нарушится последовательность блока кода, который должен был строго ей следовать. Удаление файла начнётся раньше, чем закончится чтение, а вывод результата произойдёт раньше его вычисления. 
```js
...
unlink('./input.txt');
```
Первым решением этой проблемы стали *функции обратного вызова*.
