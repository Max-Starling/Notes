...
- [Композиция как основной принцип React](#композиция-как-основной-принцип-react)
...
*Разница* — в том, *кто кого вызывает*.

**Библиотекой** (англ. `library`) называют *набор готовых решений*, которые *вызывает ваш код*: вы *сами решаете*, *когда* и *откуда обратиться* к *библиотеке*, и *сами строите приложение вокруг неё*.

**Фреймворком** (англ. `framework`) называют *готовый каркас приложения*, который *вызывает ваш код*: *фреймворк задаёт структуру проекта* и *порядок работы*, а вы *дописываете свои части* в *предусмотренных местах*.

*Инструменты обычно сами себя так и называют*: *React* на *своём сайте* назван *библиотекой* для *пользовательских интерфейсов* ([react.dev](https://react.dev/)), а *Angular* и *Vue* — *фреймворками* ([angular.dev](https://angular.dev/), [vuejs.org](https://vuejs.org/)).

*Граница при этом подвижная*: *React отвечает* только за *интерфейс*, а *маршрутизацию*, *работу с данными* и *сборку* разработчик *выбирает сам*. *Когда* эти *решения собраны вместе* и *задают структуру проекта*, *получается фреймворк*: в *документации React новое приложение* рекомендуют *начинать именно с фреймворка*, например с *Next.js* ([документация React](https://react.dev/learn/creating-a-react-app)).

## Композиция как основной принцип React

**Композицией** (англ. `composition`) называют *способ строить сложное из простого*: *компонент не наследует поведение* другого *компонента*, а *принимает его внутрь себя* или *получает в свойствах*.

*В документации React* прямо *рекомендуется использовать композицию вместо наследования*, когда *нужно переиспользовать код между компонентами* ([документация React](https://legacy.reactjs.org/docs/composition-vs-inheritance.html)).

*Самый частый приём* — *свойство* `children`: *всё*, что *вложено внутрь тега компонента*, *попадает именно в него* ([документация React](https://react.dev/learn/passing-props-to-a-component)).

```jsx
/* карточка ничего не знает о своём содержимом */
...
const Profile = () => (
  <Card title="Max-Starling">
    <Avatar />
    <p>Заметки о программировании</p>
  </Card>
);
```

*Второй приём* — *передать компонент свойством*: тогда *в одно и то же место* можно *подставлять разные части интерфейса*.
...
const Page = () => <Layout header={<Menu />} content={<Article />} />;
```

*Поэтому компоненты* можно *переиспользовать*, *не меняя их код*: *содержимое* и *поведение* задаются *снаружи*.
