...
### Элемент
**Элемент** (англ. element) в *React* имеет *такое же предназначение*, как и *HTML-элемент*: он является *строительным блоком*, при помощи которого задаётся *разметка страницы*. Но, в *отличии* от *HTML-элемета*, *React-элемент* является *обыкновенным объектом*.
