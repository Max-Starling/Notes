...
- [Жизненный цикл компонента (`component lifecycle`)](#жизненный-цикл-компонента-component-life-cycle)
...
Для полноценной работы с React достаточно просто подключить скрипт библиотеки к документу страницы (например, добавив его в `index.html`).

Иногда React называют JavaScript-фреймворком и ставят его в сравнение с такими фреймворками, как Angular и Vue. Действительно, эта троица стоит сравнения, но всё же называть React фреймворком не стоит.

### React находится в открытом доступе
Во-вторых, код библиотеки React находится в открытом доступе, а её использование на вашем проекте является абсолютно бесплатным.

### React и компоненты
В-третьих, библиотека React нацелена на облегчение написания интерактивных UI-компонент на языке JavaScript. 
...
2) `TypeScript` наделяет JavaScript статической типизацией (то есть *переменным* можно *явно задавать типы данных*, *изменение* которых *по ходу выполнения программы* *приводит к ошибке*),
...
Слева то, что пишет программист, справа то, что видит браузер, а значит и пользователь.
...
### Элемент
**React-элемент** (англ. `element`) имеет *такое же предназначение*, как и *HTML-элемент*: он является *строительным блоком*, при помощи которого задаётся *разметка страницы*. Но, в *отличие* от *HTML-элемента*, *React-элемент* является *обыкновенным объектом*.
...
Для того, чтобы понять, как React-элемент преобразуется в HTML-элемент, читайте далее про **Virtual DOM**.

<!-- 
имеет собственную объектную модель, которая называется **Virtual DOM**. 

Эта абстракция над реальным **DOM** браузера позволяет React отследить изменения React-элементов и вносить в реальный DOM только необходимые изменения 
...
При этом *атрибут* `text` (как и *другие атрибуты*, если они есть) *вместе* со своим *значением* `"Notes"` попадает в `props` *функциональной компоненты* `Title`, то есть `props = { text: 'Notes' }`, или в `this.props` *классовой компоненты* `Title`, то есть `this.props = { text: 'Notes' }`.
...
*React-компонент* выступает в роли *конструктора элементов*, то есть в роли *функции*, *строителя* (англ. `builder`), *класса*, но при этом у *компонента имеются* *жизненный цикл*, *состояние*, *поведение*, поэтому он всегда *ассоциируется* у меня с чем-то *динамическим*, *меняющимся* (*реагирующим*) *в ответ* на некоторые *внешние* и *внутренние факторы* (здесь *имеются в виду параметры компонента* `props` и *внутреннее состояние* `state`, с которыми мы *познакомимся позже*).
...
- [Жизненный цикл классового компонента](#жизненный-цикл-классового-компонента)
 ...
 - [Размонтирование классового компонента](#размонтирование-классового-компонента)
...
*Живое существо*:
1) *Рождается*,
2) *Живёт* и *изменяется* с течением времени (то есть *изменяет своё состояние* *вследствие* каких-либо *внутренних* или *внешних причин*),
3) *Умирает*.
...
<!-- (становится частью реального DOM и дерева рендера, которое затем отрисовывается браузером) -->
2) *Обновляется*, то есть *перерисовывается* при *изменениях* во *внутреннем состоянии* `state` и *внешнем состоянии* `props`.
...
* `static getDerivedStateFromProps()` - статический метод, позволяющий *обновить состояние компонента* на основании *изменившихся* `props`. Возвращает *объект с новыми значениями состояния* или `null`, если *обновление не требуется*.
* `render()`
* `componentDidMount()`
...
Компонент создаётся, изменяется и удаляется из памяти, когда в нём нет больше необходимости.

Любое изменение компонента можно отловить при помощи **методов жизненного цикла** (англ. `lifecycle methods`).

Раньше жизненный цикл компонента можно было отслеживать лишь в классовых компонентах, но с появлением React Hooks такая возможность появилась и для функциональных компонент.
...
При инициализации происходит установка Props и начального (initial) State.
...
* Хук `useEffect` выполняется *асинхронно*, а значит выполняется в фоне, не блокирует отрисовку и не создаёт задержек отрисовки, не влияет на производительность.
...
*Когда использовать*:
* Хук `useEffect` используется в 99% случаев для *большинства асинхронных задач* (*запросы к серверу* (`async..await`, `promise`), `dispatch` событий по какому-то условию, *таймеры* `setTimeout`, *подписка на события* (англ. `subscription`) и так далее).
...
```
При клике на любой элемент списка в консоли можно увидеть `(100) rendered!`. Это означает, что клик по одному элементу заставит перерендериться все остальные. Попробуем это исправить.
...
**Контролируемый компонент** (Controlled component) *контролирует данные форм* при помощи возможностей *React*. Происходит *двухстороннее связывание* при помощи `value` и `onChange` с *полями ввода*, а *данные* этих полей *должны* где-то *сохраняться* (например, в React State или Redux Store).
```jsx
...
```
Можно задать *значение по умолчанию* для *неконтролируемого компонента* при помощи свойства `defaultValue` и *получить текущее значение*, используя `ref`.
```jsx
const Form = ({ onSend }) => {
...
Виртуальный DOM позволяет отслеживать изменения React-элементов и затем вносить в реальный DOM (в html) только те изменения, которые на самом деле произошли.
...
Пример: `<div>`, `<p>`, `<Title>`, `<input>`, `<Description>` — все перечисленные элементы имеют *различные типы* и их *замена* друг на друга приведёт к *уничтожению дерева*, то есть корневого элемента вместе с его детьми.
```jsx
<!-- /* замена <div> */ -->
<div>
  <Title>Notes</Title>
...
*После обработки корневого элемента* *React* *рекурсивно проходится* по *дочерним элементам*, являющимся *корневыми* в *поддеревьях*.
...
Пусть есть список, отображающий элементы массива `items`, состоящего из чисел от 0 до 99.
```jsx
