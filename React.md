...
*React-компонент* выступает в роли *конструктора элементов*, то есть в роли *функции*, *строителя* (англ. `builder`), *класса*, но при этом у *компонента имеются* *жизненный цикл*, *состояние*, *поведение*, поэтому он всегда *ассоциируется* у меня с чем-то *динамическим*, *меняющимся* (*реагирующим*) *в ответ* на некоторые *внешние* и *внутренние фатороы* (здесь *имеются в виду параметры компонента* `props` и *внутреннее состояние* `state`, о которых мы ещё *узнаем далее*).
