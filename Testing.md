...
Например, нам в контексте теста не нужна проверка авторизации, поэтому мы заменяем реальную функцию `checkAuth()` на тестовую:
```js
...
Например, имитируем создание пользователя в базе данных (принимаем данные пользователя - возвращаем данные пользователя вместе с id, который должен был создаться в базе данных).

```js
const createUser = userData => Promise.resolve({ ...userData, _id: 1 });
```
