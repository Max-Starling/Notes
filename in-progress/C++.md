...
**Указатель** (pointer) — переменная, значением которой является адрес ячейки памяти, то есть указатель ссылается на начало какого-то блока данных из области памяти.
...
*Виртуальная функция* отличается от обычной функции тем, что для обычной функции связывание вызова функции с её определением происходит на этапе компиляции, а для виртуальных функций связывание происходит во время выполнения программы.
```cpp
class Bird {
  public:
...
Дочерние классы не должны реализовывать все виртуальные функции родителя, но должны реализовать все чистые функции.
```cpp
class Chicken: Bird {
  public:
    void eat() {
