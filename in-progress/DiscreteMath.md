...
Множества `X` называют **областью определения** (*областью задания*) **функции** `R`, а элемент`x ∈ X` - **аргументом функции**.

Множество `Y` называют **областью значений функции** `R`, а элемент `y ∈ Y` - **значением функции**.
...
Таким образом, *функция* позволяет *по заданному* аргументу `x` *однозначно определить* значение `y`.

*Функцию обозначают* *малыми латинскими буквами* (обычно `f`, `h`, `g`). После латинской буквы в *круглых скобках* записывается (передаётся) *аргумент* `x` функции, затем следуют знак равенства и значение `y` функции.

*Обозначение функции* `f`: `f(x) = y`. Также можно использовать запись: `y = f(x)`, или короче: `f(x)`, `f`.
