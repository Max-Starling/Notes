...
*Композиция функций асимметрична*: `(g ○ f) ≠ (f ○ g)`. *Порядок следования функций имеет значение*: `f(g(x) ≠ g(f(x))`.

На *примере выше* несложно показать *асимметричность композиции*: `f(g(1)) = 3 • (1 + 2) = 9`, `g(f(1)) = (3 • 1) + 2 = 5`.
