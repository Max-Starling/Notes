...
- [Кортеж (упорядоченный набор элементов)](#кортеж-упорядоченный-набор)
...
Фактически, *определения операции* и *функции нескольких переменных совпадают*. *Отличаются* лишь *обозначения*.
...
Число `n` называют **арностью операции**.

При `n = 1` *операция* называется **унарной**, при `n = 2` - **бинарной**, при `n = 3` - **тернарной**.
...
*В отличие* от *отображения* и *отношения*, *каждая операция имеет* своё *уникальное символьное представление* (*иногда* используется *несколько знаков*).

Например, *операция сложения обозначается* знаком `+`; *операция умножения* - знаками `•`, `*`, `×`; *логическое отрицание* - знаком `!` и так далее.

Говорят, что *операция* `f` **совершается над элементами** `x1 ∈ X1, x2 ∈ X2, …, xn ∈ Xn`, которые в данном случае называют **операндами**. Элемент `y ∈ Y` называют **результатом выполнения операции**. Например, в записи `1 + 3 = 4`: `+` - *операция*, `1` и `3` - *операнды*, `4` - *результат выполнения операции*.

В *школьной математике* чаще всего встречаются *арифметические операции*, в *программировании* - *логические операции*, в *математическом анализе* - *операции дифференцирования и интегрирования* и так далее.

### Бинарная операция

### Свойства бинарных операций

Как *разновидность отношений*, *операции* могут *обладать* или *не обладать* некоторыми **свойствами**, которые *определяют поведение операций*. Тем не менее, *свойства операций* несколько *отличаются* от *свойств отношений*.

Для *рассмотрения свойств* *не* столь *важно*, что *обозначения всех операций различны*. *Условно обозначим операцию символом* `◇`, *ещё одну операцию* (при её необходимости) *обозначим* символом `◆`.

1) **Коммутативность** (*переместительное свойство*) — такое *свойство* операции, при котором *порядок следования операндов неважен*: `x ◇ y = y ◇ x`. Например, операция *сложения коммутативна*: `1 + 7 = 7 + 1 = 8`.
2) **Антикоммутативность** — такое *свойство* операции, при котором `(x ◇ y) = -(y ◇ x)`. Например, операция *вычитания антикоммутативна*: `8 − 1 = −(1 − 8) = 7`.
3) **Ассоциативность** (*сочетательное свойство*) — такое *свойство* операции, при котором *расстановка скобок не имеет эффекта* (тогда *скобки можно опустить*): `(x ◇ y) ◇ z = x ◇ (y ◇ z) = x ◇ y ◇ z`. Например, операция *сложения ассоциативна*: `(3 + 2) + 2 = 3 + (2 + 2) = 3 + 2 + 2 = 6`.
4) **Дистрибутивность** (*распределительное свойство*). Операция `◆` - **дистрибутивна относительно операции** `◇`, если выполняется `(x ◇ y) ◆ z = x ◆ z ◇ y ◆ z`. Например, операция *умножения дистрибутивна относительно* операции *сложения*: `(1 + 7) • 3 = 1 • 3 + 7 • 3 = 24`.
5) **Идемпотентность** — такое *свойство* операции, при которотом *повторное применение операции не имеет* никакого *эффекта*. Например, *модуль числа идемпотентен*: `|-1| = ||-1|| = |||-1||| = 1`.

### Приоритет операций

Операции обладают **приоритетом** - очерёдность выполнения по сравнению с другими операциями.
...
### Задача (свойства операций над множествами)
