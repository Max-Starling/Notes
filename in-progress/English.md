...
Действия времени Present Perfect привязаны к настоящему времени своим результатом. 
...
```
* Не имеют временных форм (кроме `can` – `could`, `may` – `might`, `have to` и `be to`) и страдательного залога.
...
*Вспомогательные глаголы* в большинстве языков обычно *используются* для *образования временных* и *залоговых форм*.

Чаще всего вспомогательным является обычный глагол, который просто употребляется не в основном своём значении. Если глагол используется как вспомогательный, то он лишается своего лексического значения, то есть больше не означает то, что он обычно значит, а просто указывает на какое-то время или какую-то конструкцию.
...
1) Избежать повторения основного глагола (в данном случае вспомогательный глагол служит чем-то вроде переменной, местоимения)
```
...
```
4) Показать, что ваше мнение (опыт) *совпадает* с мнением (опытом) говорящего. Для этого используется *конструкция* `so + auxiliary verb + subject` c *положительным глаголом* (англ. `positive verb`) и конструкция `neither (nor) + auxiliary verb + subject` для отрицательных глаголов (англ. `negative verb`). Это также работает и с модальными глаголами.
```js
...
```
7) Чтобы *уточнить (проверить)* какую-то *информацию*, к *утвердительному предложению* можно *добавить вспомогательную конструкцию*, которая *сделает* из него *вопрос* и которую на *русский язык* можно *перевести* `"не так ли?"`. На такой *тип вопроса* можно *ответить либо "да"*, *либо "нет"* - *других вариантов ответа не подразумевается*. *Конструкция* такого предложения следующая: `auxiliary verb + subject` для *отрицательных глаголов* в *утвердительной части предложения* и `auxiliary verb + not subject` для *положительных глаголов*.
```js
...
Предложения бывают простыми и составными. Составное предложение состоит из нескольких частей, представляющих собой простые предложения.
...
Полное предложение состоит из подлежащего и сказуемого. Дополнительно в предложении могут присутствовать второстепенные члены предложения: дополнение, определение и обстоятельство.
...
```
Но бывают и исключения, когда сказуемое выражено существительным
```
...
* В конце предложения обычно ставится знак вопроса `?` (англ. `question mark`)
...
Построение вопроса к подлежащему совпадает с *построением утвердительного предложения*, поэтому структуры *вопроса* и *ответа* одинаковы: в ответе заменяется лишь вопросительное слово.
...
*Косвенный вопрос* обычно состоит из двух частей. Первая часть вежливо указывает на то, что будет задан вопрос. Вторая часть раскрывает основной вопрос, причину обращения.
...
**Условное предложение** (англ. `Conditional sentence`) - это сложное предложение, которое состоит из двух частей: **условия** (англ. `condition`, `if-clause`) и **следствия, результата** (англ. `result`, `other clause`) *выполнения* данного *условия*.
...
При использовании `First condition` *условие* имеет *любую форму настоящего времени*, а *следствие* - *любую форму будущего времени*:
```jsx
...
При использовании второго условного предложения *условие* имеет *прошедшее время* (обычно `Past Simple`, но иногда можно использовать `Past Continuous`), а *следствие* содержит глаголы `would/could/might`, выражающие *возможность в прошедшем времени* (*будущее в прошлом*):
```jsx
...
Сам глагол `wish` *отражает желание* (*пожелание*).  
...
Построение предложения с данной конструкцией зависит от времени (настоящее/прошедшее/будущее) и от того, совпадают ли субъекты в обеих частях предложения.
...
Если мы хотим чему-то отдать предпочтение в прошедшем времени, глагол `verb` нужно использовать с временной *формой* `<Perfect>`. Причём, если `SubjectA` и `SubjectB` *совпадают*, то *используется* `<Present Perfect>`, а если не совпадают, то `Past Perfect`. На прошедшее время указывают определённые слова (например, `then`).
```js
...
Конструкция `used to + infinitive` используется для того, чтобы сослаться на наши привычные (повторяющиеся) действия, состояния или ситуации в прошлом, которые сейчас уже не действительны. То есть данная конструкция подразумевает контраст между тем, что было раньше и тем, что есть сейчас. 
...
Предложение с конструкцией `used to` может состоять как из одной части, так и из двух. Вторая часть, отражающая контраст и начинающаяся обычно со слова "но" (англ. `but`), может иногда опускаться.

*Не перепутайте конструкции* `used to + infinitive` и `be used to + gerund` (о ней будет дальше)!
...
И `used to`, и `would` могут *использоваться* для *рассказа* о *привычках прошлого*. *Иногда* они *используются вместе* в *предложении*, тогда обычно `used to` выходит вперёд, чтобы *задать сцену рассказа*.
...
*В отличие* от `would` конструкция `used to` используется для *описания ситуации*, которая *уже не актуальна*.
```cpp
// Можно сказать:
...
Конструкция `be used to + gerund` означает "привыкнуть к чему-то" (англ. `be accustomed to`) или "быть знакомым с чем-то" (англ. `be familiar with`). То есть для вас что-то было в новинку, но теперь вы кое-как привыкли к этому, познакомились, освоились.
...
2) Разница в применимости: `used to` используется только в прошлом, `be (get) used to` может использоваться в любом времени.
...
В своём *учебнике по английскому* я также нашёл *ещё один пункт между* `7) Color` и `8) Origin`: `7.5) Pattern` - шаблон, узор (например: полосатый, "в полосочку" - `striped`; клетчатый, "в клеточку" - `checkered`; пунктирный, "в горошек" - dotted).
```
