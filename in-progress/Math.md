...
`A = { x, y, z }`, где `A` - произвольная заглавная латинская буква, являющнйся чем-то вроде переменной (ссылки) на множество, а внутри фигурных скобок через запятую перечисляются элементы множества (в данном случае ими являются объекты `x`, `y` и `z`).
