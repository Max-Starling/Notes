...
- [О бинарных и унарных операциях над множествами](#о-бинарных-и-унарных-операциях-над-множествами)
...
Существует *два основных типа операций над множествами*: *бинарная* и *унарная*.
...
При `n = 1` *отношение* называется **унарным**. Оно *приравнивается* к *свойству* одного *объекта* и обычно не рассматривается.
...
При `n = 3` *отношение* называют **тернарным** и так далее.
...
Разберёмся с *определением*. Вспомним, что *декартово произведение* `A × B` - это *множество всех упорядоченных пар* элементов `x ∈ A` и `y ∈ B`. Значит некоторое *подмножество* `R` *декартового произведения* - множество, *содержащее определённое количество* этих *упорядоченных пар*. Запись `(x, y) ∈ R` наглядно показывает *принадлежность упорядоченной пары* к множеству `R`. Эта принадлежность и означает, что элементы `x` и `y` *связаны отношением*. 
...
Например, *равенство* `=` является *отношением эквивалентности*, поскольку *для любых* объектов `x`, `y` и `z` выполняется:  
1) `x = x`,  
2) `(x = y) ⇒ (y = x)`,  
3) `(x = y) ∧ (y = z) ⇒ (x = z)`.
...
*Множество* всех *первых элементов* `x` *упорядоченных пар* `(x, y)` из множества `R` называют **областью определения** отношения `R`.

*Обозначение области определения*: `Dom R = { x ∣ ∃y ((x, y) ∈ R) }`, то есть множество таких элементов `x`, для которых найдутся такие элементы `y`, что упорядоченная пара элементов `x` и `y` будет принадлежать множеству `R` (`(x, y) ∈ R`).

*Множество* всех *вторых элементов* `y` *упорядоченных пар* `(x, y)` из множества `R` называют **областью значений** отношения `R`.

*Обозначение области значений*: `Im R = { y ∣ ∃x ((x, y) ∈ R) }`, то есть множество таких элементов `y`, для которых найдутся такие элементы `x`, что упорядоченная пара элементов `x` и `y` будет принадлежать множеству `R` (`(x, y) ∈ R`).

<!--
Таким образом, в `xRy` слева область определения, справа - область значений.
-->

### Функция и биекция

**Функция одной переменной** (отображение) — *бинарное отношение* `R`, *определённое* на *некотором множестве* `X`, такое, что *каждому* значению `x` отношения `xRy` *соответствует единственное* значение `y`. 

Следующая *аксиома* передаёт *свойство функциональности* отношения `R`: `(xRy ∧ xRz) → (y ≡ z)`, то есть, если *одновременно выполняются* `xRy` и `xRz`, то `y = z` (*не может существовать двух разных значений* для *одного* `x`.

Таким образом, *функция* позволяет *по заданному* значению `x` *однозначно определить* значение `y`.

*Обозначение функции* `f`: `f(x) = y`.

**Биекция** (взаимно-однозначное отношение) — *бинарное отношение* `R`, *определённое* на некотором множестве, такое, что *каждому* значению `x` *соответствует единственное* значение `y`, а *каждому* значению `y` *соответствует единственное* значение `x`.
