...
Eсли множества `A` и `B` *не пересекаются* (`A ⋂ B = ∅`), то их *разность* `A / B = A` и `B / A = B`.
...
**Упорядоченная пара элементов** - *пара элементов*, в которой *порядок следования элементов имеет значение*. 

*Обозначение упорядоченной пары* элементов `x` и `y`: `(x, y)`. 

Элемент `x` называют **первой координатой** (первой компонентой) *пары*, элемент `y` – **второй координатой** (второй компонентой) *пары*.
...
Обозначение *декартова произведения* `A` и `B`:  
`A × B = { ( x , y ) ∣ "x ∈ A и y ∈ B" }`.

Поскольку *пары упорядочены* - *порядок имеет значение*. Поэтому `(x, y) ≠ (y, x)`, а значит и `(A × B) ≠ (B × A)`.

*Количество пар* в множестве `A × B` *равняется произведению количества элементов* множества `A` *на количество элементов* множества `B`.

### Задача #5 (декартово произведение)
«Заданы множества `A = { x, y, z }`, `B = { 0, 1 }`. Найти их декартово произведение.»

Ответ: `A × B = { (x, 0), (x, 1), (y, 0), (y, 1), (z, 0), (z, 1) }`, `B × A = { (0, x), (0, y), (0, z), (1, x), (1, y), (1, x) }`.
