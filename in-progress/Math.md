...
Объявляют множество следующим образом:

`A = { x, y, z }`, где `A` - произвольная заглавная латинская буква, являющаяся чем-то вроде переменной (ссылки) на множество, а внутри фигурных скобок через запятую перечисляются элементы множества (в данном случае ими являются объекты `x`, `y` и `z`).
...
Примеры: символ `i` принадлежит множествам букв белорусского и латинского алфавитов, но не принадлежит множеству букв русского алфавита; число `0` принадлежит множеству целых чисел, но не принадлежит множеству натуральных чисел.

## Основные определения множеств

*Множества* `A` и `B` называют **равными**, если *все* их *элементы совпадают*. Иначе говоря, множество `A` содержит в себе все элементы множества `B`, а множество `B` содержит в себе все элементы множества `A`. Обозначение: `A = B`.
...
Задать элементы множества можно двумя способами:  
* перечислить все элементы через запятую в определении множества,
* указать признак (свойство), по которому эти элементы можно найти (получить).
