...
Эту проблему решает HTTPS (HTTP Secure) — безопасная версия протокола, использующая протокол TLS или SSL для шифрования.
...
время как сообщение передаётся по сети.
...
HEAD /user/friends
```
При помощи *метода HEAD* можно получить мета-данные, хранящиеся в заголовке ответа (Response Headers), и при этом напрасно не передавать данные с сервера.
...
При создании нового ресурса подразумевается создание нового подчинённого ресурса (subordinate) для ресурса, заданного по URL. Например, в случае примера выше это создание нового пользователя для ресурса `users`, содержащего информацию о пользователях.

*Статус-коды POST-запроса*
...
PUT-запросы отвечают статусом 200, если ресурс успешно обновлён, и 404, если ресурс не найден.

### DELETE

**Метод DELETE** используется для удаления ресурса, указанного в URL.  

На запросы DELETE в ответ возвращается код состояния 200 (ОК), если он был успешно  
удалён, или 404 (НЕ НАЙДЕН), если удаляемый ресурс не может быть найден.
...
Таким образом, запросы GET считаются безопасными операциями, потому что вызов их один  
...
результат остаётся тем же: удалённый ресурс.
...
Это просто причудливый способ сказать, что REST даёт вам набор ограничений для  
