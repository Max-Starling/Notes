...
**Зависимостями** (англ. `dependencies`) в приложении выступают *сторонние пакеты* (модули), которые *декларируются* в `package.json`. Чаще всего зависимости скачиваются из [NPM Registry](https://www.npmjs.com/) 
...
```
то `package.json` вашего приложения после установки `foo` и его одноранговых зависимостей с помощью `npm install` должен выглядеть примерно следующим образом
```js
{
  "name": "my-awesome-app",
  "version": "1.0.0",
...
Указание свойства `overrides` (рус. `переопределения`) позволяет *принудительно установить* (*перезаписывать*) *версии* некоторых *пакетов* в приложении.
