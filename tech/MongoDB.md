...
```
Для того, чтобы найти документ, который содержит определённого пользователя в массиве, следует использовать свойтсво `$elemMatch`, которое использует в качестве значения вложенный query-запрос.
