...
Сохраняя информацию о запросе в коллекцию, вы можете в любой момент переиспользовать её в качестве темплейта для новых запросов или же периодически вызывать один и тот же запрос в один клик в любое время.

У коллекции также есть свои настройки, которые применяются ко всем запросам в ней:
...
но можно также для каждого проекта создавать новое рабочее пространство для наилучшей изолированности (вдруг вы собираетесь давать доступ кому-то к воркспейсам):
* Users dashboard
  * Production
    * Get users - GET https://users-dashboard.com/api/users
    * Get user by id - GET http://users-dashboard.com/api/users/:id
...
например, вместо того, чтобы создавать две коллекции `Users dashboard Local`, `Users dashboard Production`, достаточно просто завести переменную `base_url`.
```
