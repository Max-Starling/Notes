...
## Конкатенация `concatenation` при помощи `join`
...
Составные типы данных (например, состоящие из нескольких значений, возможно, разных типов данных) импортируются из библиотеки `typing`:
...
```
* `Optional` означает, что значение задавать необязательно. Если значение не задать, ошибки не будет.
```py
...
Используйте его, если *порядок элементов важен*.

Например, список призёров турнира, номера камер хранения в магазине.

#### Множество

Set - неупорядоченный набор уникальных элементов. Используйте его, если порядок не важен - важна уникальность элементов.

Например, список имён всех авторов комментариев под постом в некой социальной сети (один человек может оставить комментарий сколько угодно раз, но в список его имя попадёт лишь единожды).
...
Лямбда-функция возведения в степень.
```py
