...
# /app
```
* `ssh <username>@<host>` - вход на сервер по ssh при помощи имени пользователя и адреса хоста (интерактивный режим).
...
Поскольку `#` является символом начала комментария во многих скриптовых языках программирования, строка с шебангом обычно пропускается интерпретатором.
