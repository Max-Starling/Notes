...
**Python Package Index (PyPI)** - это *репозиторий*, *хранящий программное обеспечение* для *языка* Python в виде *пакетов*. *Пакеты находятся* в *открытом доступе* и *разработаны сообществом* Python-разработчиков.

*Аналогом* `PyPI` для языка *JavaScript* является *репозиторий* **NPM Registry**.

## О менеджере пакетов PIP

**PIP** (англ. `Package Installer for Python`) - это *менеджер пакетов* для языка Python, который позволяет устанавливать их из репозитория PyPI и затем управлять ими при помощи консольной команды `pip`.

*Аналогом* `PIP` является *менеджер пакетов* **NPM** (англ. `NodeJS Package Manager`).
...
Если `python` запускается в терминале, то всё хорошо.  
...
Если Python уже установлен и добавлен в переменные окружения, то достаточно ввести в терминале ОДНУ из следующих команд (`py` или `python` *зависит* от *версии* `Python`):
```css
